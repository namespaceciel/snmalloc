...
void memcpy_with_align_offset(size_t source_alignment, size_t source_offset, size_t dest_alignment, size_t dest_offset,
                              std::string data) {
    ...
    auto src_        = ::operator new(data.size() + source_offset, std::align_val_t{source_alignment});
    auto dst_        = ::operator new(data.size() + dest_offset, std::align_val_t{dest_alignment});
...
static constexpr size_t size_limit = 16384;
...
struct Result {
    ...
    void check() {
        auto res = std::reduce(std::execution::unseq, ptr, ptr + size, static_cast<unsigned char>(0),
...
void snmalloc_random_walk(std::vector<std::tuple<unsigned, size_t, char>> payload) {
    ...
    for (auto& p : payload) {
        ...
        switch (e.kind) {
            case EventKind::AllocZero : {
                auto ptr = static_cast<char*>(scoped->alloc<snmalloc::YesZero>(e.size_or_index));
                results.emplace_back(0, ptr, e.size_or_index);
                break;
            }

            case EventKind::AllocNoZero : {
                auto ptr = static_cast<char*>(scoped->alloc<snmalloc::NoZero>(e.size_or_index));
            ...
            case EventKind::ReFill : {
                ...
                auto index = e.size_or_index % results.size();
                std::fill(results[index].ptr, results[index].ptr + results[index].size, e.filler);
...
FUZZ_TEST(snmalloc_fuzzing, memcpy_with_align_offset)
    .WithDomains(fuzztest::InRange(0, 6), fuzztest::Positive<size_t>(), fuzztest::InRange(0, 6),
                 fuzztest::Positive<size_t>(), fuzztest::String());
