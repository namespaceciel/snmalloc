...
private:
    ...
    static_assert(STATE_BITS >= RESULT_BITS, "STATE must have at least as many bits as RESULT");

    STATE x;
    STATE y;

    inline static STATE rotl(STATE x, STATE k) {
        return (x << k) | (x >> (STATE_BITS - k));
    }

public:
    XorOshiro(STATE x_ = 5489, STATE y_ = 0)
        : x(x_), y(y_) {
