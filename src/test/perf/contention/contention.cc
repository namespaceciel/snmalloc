...
void test_tasks_f(size_t id) {
    ...
    for (size_t n = 0; n < swapcount; n++) {
        ...
        size_t* out = contention[n % swapsize].exchange(res, std::memory_order_acq_rel);
...
void test_tasks(size_t num_tasks, size_t count, size_t size) {
    ...
    for (size_t n = 0; n < size; n++) {
        ...
        size_t* res       = (size_t*)(use_malloc ? malloc(alloc_size) : a.alloc(alloc_size));
        *res              = alloc_size;
        contention[n]     = res;
    }
    swapcount = count;
...

    {
        ...
        std::cout << "Task test, " << num_tasks << " threads, " << count << " swaps per thread " << test.time()
                  << " ticks" << std::endl;
...
int main(int argc, char** argv) {
    ...
    std::cout << "Allocator is " << (use_malloc ? "System" : "snmalloc") << std::endl;
