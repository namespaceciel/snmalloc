...
static constexpr bool be_chatty = false;
...
void consumer(const struct params* param, size_t qix) {
    ...
    do {
        ...
        if (myq.can_dequeue(domesticate_nop, domesticate_nop)) {
            myq.dequeue(domesticate_nop, domesticate_nop, [qix, &a, &reap](freelist::HeadPtr o) {
                UNUSED(qix);
    ...
    } while (myq.can_dequeue(domesticate_nop, domesticate_nop) || producers_live || (queue_gate > param->N_CONSUMER));
...
void proxy(const struct params* param, size_t qix) {
    ...
    do {
        if (myq.can_dequeue(domesticate_nop, domesticate_nop)) {
            myq.dequeue(domesticate_nop, domesticate_nop, [qs, qix, &r](freelist::HeadPtr o) {
        ...
        std::this_thread::yield();
    } while (myq.can_dequeue(domesticate_nop, domesticate_nop) || producers_live || (queue_gate > qix + 1));
...
void producer(const struct params* param, size_t pix) {
    ...
    for (size_t batchix = param->N_PRODUCER_BATCH; batchix > 0; batchix--) {
        ...
        for (size_t msgix = 0; msgix < nmsg; msgix++) {
            ...
            auto msgc = capptr::Alloc<void>::unsafe_from(msg).template as_reinterpret<freelist::Object::T<>>();
            batch.add(msgc, msgqueue_key, msgqueue_key_tweak);
        }

        /* Post to random queue */
        auto [bfirst, blast] = batch.extract_segment(msgqueue_key, msgqueue_key_tweak);
...
int main(int argc, char** argv) {
    ...
    std::cout << "msgpass --producers=" << param.N_PRODUCER << " --consumers=" << param.N_CONSUMER
              << " --proxies=" << param.N_PROXY << " --batches=" << param.N_PRODUCER_BATCH
              << " --max-out=" << param.N_MAX_OUTSTANDING << " --max-batch=" << param.N_MAX_BATCH_SIZE << std::endl;

    param.N_QUEUE  = param.N_CONSUMER + param.N_PROXY;
    param.msgqueue = new FreeListMPSCQ<msgqueue_key, msgqueue_key_tweak>[param.N_QUEUE];
