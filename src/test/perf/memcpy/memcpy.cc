...
template<typename Memcpy>
void test(size_t size, Memcpy mc, std::vector<std::pair<size_t, std::chrono::nanoseconds>>& stats) {
...
int main(int argc, char** argv) {
    ...
    std::vector<std::pair<size_t, std::chrono::nanoseconds>> stats_sn, stats_sn_checked, stats_platform,
        stats_platform_checked;
    ...
    for (auto repeat = repeats; 0 < repeat; repeat--) {
        ...
        for (size_t i = 0; i < stats_sn.size(); i++) {
            ...
            std::cout << s1.first << ", " << s1.second.count() << ", " << s2.second.count() << ", " << s3.second.count()
