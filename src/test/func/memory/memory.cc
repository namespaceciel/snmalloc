...
#include <vector>
#if ((defined(__linux__) && !defined(__ANDROID__)) || defined(__sun)) && !defined(SNMALLOC_QEMU_WORKAROUND)
/*
...
void test_external_pointer() {
    ...
    for (snmalloc::smallsizeclass_t sc = size_to_sizeclass(MIN_ALLOC_SIZE); sc < NUM_SMALL_SIZECLASSES; sc++) {
        size_t size = sizeclass_to_size(sc);
        void* p1    = alloc.alloc(size);

        if (size != alloc.alloc_size(p1)) {
            std::cout << "Requested size: " << size << " alloc_size: " << alloc.alloc_size(p1) << std::endl;
            abort();
        }
        ...
        for (size_t offset = 0; offset < size; offset += 17) {
            ...
            if (p1 != p3) {
                std::cout << "size: " << size << " alloc_size: " << alloc.alloc_size(p1) << " offset: " << offset
                          << " p1: " << p1 << "  p3: " << p3 << std::endl;
            }
            SNMALLOC_CHECK(p1 == p3);
            if ((size_t)p4 != (size_t)p1 + size - 1) {
                std::cout << "size: " << size << " end(p4): " << p4 << " p1: " << p1
                          << "  p1+size-1: " << pointer_offset(p1, size - 1) << std::endl;
            }
...
void check_offset(void* base, void* interior) {
    ...
    if (calced_base != (void*)base) {
        std::cout << "Calced base: " << calced_base << " actual base: " << base << " for interior: " << interior
                  << std::endl;
        abort();
    }
}
...
void test_external_pointer_large() {
    ...
    std::cout << "Total size allocated in test_external_pointer_large: " << total_size << std::endl;
...
void test_external_pointer_stack() {
    ...
    for (size_t i = 0; i < stack.size(); i++) {
        if (alloc.external_pointer(&stack[i]) > &stack[i]) {
            std::cout << "Stack pointer: " << &stack[i] << " external pointer: " << alloc.external_pointer(&stack[i])
                      << std::endl;
            abort();
        }
    }
...
void test_remaining_bytes() {
    ...
    for (snmalloc::smallsizeclass_t sc = size_to_sizeclass(MIN_ALLOC_SIZE); sc < NUM_SMALL_SIZECLASSES; sc++) {
        ...
        for (size_t offset = 0; offset < size; offset++) {
            ...
            if (rem != (size - offset)) {
                printf("Allocation size: %zu,  Offset: %zu,  Remaining bytes: %zu, "
                       "Expected: %zu\n",
                       size, offset, rem, size - offset);
                abort();
            }
        }
        alloc.dealloc(p);
...
int main(int argc, char** argv) {
    ...
    test_limited(8 * GiB, // 8 * GiB is large enough for a loose upper-bound of our allocations
                 count);
    if (count) {
