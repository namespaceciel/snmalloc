...
struct Sandbox {
    ...
    using InternalAlloc     = LocalAllocator<InternalCoreAlloc, no_op_register_clean_up>;
    ...
    Sandbox(size_t sb_size)
        : start(alloc_sandbox_heap(sb_size)),
          ...
          state(pointer_offset(CapPtr<void, CBChunk>(start), sizeof(SharedState)), sb_size - sizeof(SharedState)),
        ...
        auto* state_proxy       = static_cast<MemoryProviderProxy*>(alloc.alloc(sizeof(MemoryProviderProxy)));
        ...
        internal_alloc = new (alloc.alloc(sizeof(InternalAlloc))) InternalAlloc(*state_proxy);
    }
    ...
    bool is_in_sandbox_heap(void* ptr, size_t sz) {
        return (ptr >= pointer_offset(start, sizeof(SharedState)) && (pointer_offset(ptr, sz) < top));
    }
