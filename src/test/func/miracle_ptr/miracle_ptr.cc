...
using Alloc =
    snmalloc::LocalAllocator<snmalloc::StandardConfigClientMeta<ArrayClientMetaDataProvider<std::atomic<size_t>>>>;
} // namespace snmalloc
...
inline void free(void* ptr) {
    ...
    auto previous = snmalloc::libc::get_client_meta_data(ptr).fetch_add((size_t)-1);
...
inline void acquire(void* p) {
    auto previous = snmalloc::libc::get_client_meta_data(p).fetch_add((size_t)2);
