...
struct NodeRef {
    ...
    constexpr NodeRef(size_t* p)
        : ptr(p) {}

    constexpr NodeRef()
        : ptr(nullptr) {}

    constexpr NodeRef(const NodeRef& other)
        : ptr(other.ptr) {}

    constexpr NodeRef(NodeRef&& other)
        : ptr(other.ptr) {}
    ...
    for (size_t i = 0; i < 20 * size; i++) {
        ...
        if (op < 2 || first) {
            ...
            for (auto j = batch; j > 0; j--) {
                ...
                entries.erase(std::remove(entries.begin(), entries.end(), min), entries.end());
                if (s != entries.size() + 1) {
