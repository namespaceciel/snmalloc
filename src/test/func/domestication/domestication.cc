...
public:
    ...
    using ConcreteAuthmap = FlatPagemap<MinBaseSizeBits<Pal>(), capptr::Arena<void>, Pal, false>;

    using Authmap = DefaultAuthmap<ConcreteAuthmap>;

public:
    using Base = Pipe<PalRange<Pal>, PagemapRegisterRange<Pagemap>, PagemapRegisterRange<Authmap>>;

    using LocalState = StandardLocalState<Pal, Pagemap, Base>;

    using GlobalPoolState = PoolState<CoreAllocator<CustomConfig>>;

    using Backend = BackendAllocator<Pal, PagemapEntry, Pagemap, Authmap, LocalState>;
...
public:
    ...
    inline static bool domesticate_trace;
    inline static size_t domesticate_count;
    inline static uintptr_t* domesticate_patch_location;
    inline static uintptr_t domesticate_patch_value;

    /* Verify that a pointer points into the region managed by this config */
    template<typename T, SNMALLOC_CONCEPT(capptr::IsBound) B>
    static CapPtr<T, typename B::template with_wildness<capptr::dimension::Wildness::Tame>>
...
#  endif
        ...
        if (domesticate_patch_location != nullptr
            && p.template as_reinterpret<uintptr_t>().unsafe_ptr() == domesticate_patch_location) {
        ...
        return CapPtr<T, typename B::template with_wildness<capptr::dimension::Wildness::Tame>>::unsafe_from(
            p.unsafe_ptr());
    }
};
...
int main() {
    static constexpr bool pagemap_randomize = mitigations(random_pagemap) & !aal_supports<StrictProvenance>;
    ...
    snmalloc::CustomConfig::domesticate_patch_location = static_cast<uintptr_t*>(p);
     ...
     *     after q).
     */
    static constexpr size_t expected_count = snmalloc::CustomConfig::Options.QueueHeadsAreTame ? 2 : 3;
