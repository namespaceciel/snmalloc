...
void check_get(bool bounded, address_t address, T expected, const char* file, size_t lineno) {
    T value = 0;
    if (bounded) {
    ...
    if (value.v != expected.v) {
        std::cout << "Location: " << std::hex << address << " Read: " << value.v << " Expected: " << expected.v
                  << " on " << file << ":" << lineno << std::endl;
        failure_count++;
    }
}
...
void test_pagemap(bool bounded) {
    ...
    T value = 1;
    for (address_t ptr = low; ptr < high; ptr += bits::one_at_bit(GRANULARITY_BITS + 3)) {
