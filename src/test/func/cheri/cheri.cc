...
#  if defined(__FreeBSD__)
        static constexpr int irm = MINCORE_INCORE | MINCORE_REFERENCED | MINCORE_MODIFIED;
        char ic[2];
...
#  endif
    ...
    {
        static_assert(std::is_same_v<decltype(alloc.alloc), LocalAllocator<StandardConfig>>);
    ...
    message("Checking memcpy behaviors");
    {
        ...
        for (size_t pre = 1; pre < sizeof(int*); pre++) {
            for (size_t post = 0; post < sizeof(int*); post++) {
                for (size_t ptrs = 0; ptrs < ncaps - 2; ptrs++) {
                    ...
                    snmalloc::memcpy<false>(pointer_offset(ocaps, pre), pointer_offset(icaps, pre),
                                            (ptrs + 1) * sizeof(int*) - pre + post);

                    /* prefix */
                    SNMALLOC_CHECK(memcmp(pointer_offset(icaps, pre), pointer_offset(ocaps, pre), sizeof(int*) - pre)
                                   == 0);
                    /* pointer */
                    for (size_t p = 0; p < ptrs; p++) {
                        SNMALLOC_CHECK(__builtin_cheri_tag_get(ocaps[1 + p]));
                        SNMALLOC_CHECK(__builtin_cheri_equal_exact(icaps[1 + p], ocaps[1 + p]));
                    }
                    /* suffix */
        ...
        for (size_t sa = 0; sa < sizeof(int*); sa++) {
            for (size_t da = 0; da < sizeof(int*); da++) {
                ...
                snmalloc::memcpy<false>(pointer_offset(ocaps, da), pointer_offset(icaps, sa),
                                        n * sizeof(int*) - da - sa);
                ...
                SNMALLOC_CHECK(memcmp(pointer_offset(icaps, sa), pointer_offset(ocaps, da), n * sizeof(int*) - da - sa)
                               == 0);
            }
        }
    }
