...
void check_result(size_t size, size_t align, void* p, int err, bool null) {
    bool failed = false;
    EXPECT((errno == err) || (err == SUCCESS), "Expected error: {} but got {}", err, errno);
    if (null) {
...
#  endif
    ...
    if (exact_size && (alloc_size != expected_size) && (size != 0)) {
        INFO("Usable size is {}, but required to be {}.", alloc_size, expected_size);
        failed = true;
    }
    if ((!exact_size) && (alloc_size < expected_size)) {
        INFO("Usable size is {}, but required to be at least {}.", alloc_size, expected_size);
        failed = true;
    }
    ...
    if ((address_cast(p) % natural_alignment(size)) != 0) {
        INFO("Address is {}, but should have natural alignment to {}.\n", p, natural_alignment(size));
        failed = true;
    }
...
void test_reallocarr(size_t size_old, size_t nmemb, size_t size, int err, bool null) {
    void* p = nullptr;
...
int main(int argc, char** argv) {
    ...
    MessageBuilder<1024> b{"testing pointer {} size_t {} message, {} world, null is {}, -123456 is "
                           "{}, 1234567 is {}",
                           fakeptr,
                           size_t(42),
                           "hello",
                           ...
                           -123456,
                           1234567};
    if (strcmp("testing pointer 0x42 size_t 0x2a message, hello world, null is 0x0, "
    ...
    for (smallsizeclass_t sc = 0; sc < NUM_SMALL_SIZECLASSES; sc++) {
        ...
        bool overflow = false;
        for (size_t n = 1; bits::umul(size, n, overflow) <= MAX_SMALL_SIZECLASS_SIZE; n *= 5) {
            if (overflow) {
                break;
            }
    ...
    for (size_t align = sizeof(uintptr_t); align < MAX_SMALL_SIZECLASS_SIZE * 8; align <<= 1) {
    ...
    EXPECT(our_malloc_usable_size(nullptr) == 0, "malloc_usable_size(nullptr) should be zero");
