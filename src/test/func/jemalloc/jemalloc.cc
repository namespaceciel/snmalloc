...
void check_lg_align_macro() {
    static_assert(OUR_MALLOCX_LG_ALIGN(Size) == MALLOCX_LG_ALIGN(Size), "Our definition of MALLOCX_LG_ALIGN is wrong");
    static_assert(OUR_MALLOCX_LG_ALIGN(Size) == ALLOCM_LG_ALIGN(Size), "Our definition of ALLOCM_LG_ALIGN is wrong");
    static_assert(JEMallocFlags(Size).log2align() == Size, "Out log2 align mask is wrong");
    if constexpr (Size > 0) {
 ...
 */
template<void*(Mallocx)(size_t, int), void(Dallocx)(void*, int), size_t(Sallocx)(const void*, int),
...
void test_size() {
    ...
    test_sizes_and_alignments([](size_t size, int align) {
        ...
        EXPECT(ptr != nullptr, "Failed to allocate {} bytes with {}-bit alignment", size, align);
        size_t allocated = Sallocx(ptr, 0);
        EXPECT(allocated == expected, "Expected to have allocated {} bytes, got {} bytes", expected, allocated);
        Dallocx(ptr, 0);
    });
}
 ...
 */
template<void*(Mallocx)(size_t, int), void(Dallocx)(void*, int), void*(Rallocx)(void*, size_t, int)>
void test_zeroing() {
    ...
    auto test = [](size_t size, int align) {
        ...
        EXPECT(ptr != nullptr, "Failed to reallocate for {} byte allocation", size * 2);
        EXPECT(ptr[size] == 0,
 ...
 */
template<void*(Mallocx)(size_t, int), void(Dallocx)(void*, int), size_t(Xallocx)(void*, size_t, size_t, int)>
void test_xallocx() {
...
template<int(Allocm)(void**, size_t*, size_t, int), int(Sallocm)(const void*, size_t*, int), int(Dallocm)(void*, int),
...
void test_nallocm_size() {
    ...
    test_sizes_and_alignments([](size_t size, int align) {
        ...
        EXPECT((ret == ALLOCM_SUCCESS), "nallocm({}, {}) failed with error {}", size, flags, ret);
        void* ptr;
        size_t allocated;
        ret = Allocm(&ptr, &allocated, size, flags);
        EXPECT((ptr != nullptr) && (ret == ALLOCM_SUCCESS), "Failed to allocate {} bytes with {} bit alignment", size,
               align);
        EXPECT(allocated == expected, "Expected to have allocated {} bytes, got {} bytes", expected, allocated);
        ret = Sallocm(ptr, &expected, 0);
        EXPECT((ret == ALLOCM_SUCCESS) && (allocated == expected), "Expected to have allocated {} bytes, got {} bytes",
               expected, allocated);
...
template<int(Allocm)(void**, size_t*, size_t, int), int(Rallocm)(void**, size_t*, size_t, size_t, int),
...
void test_rallocm_nomove() {
    ...
    test_sizes_and_alignments([](size_t size, int align) {
        ...
        EXPECT((ret == ALLOCM_ERR_NOT_MOVED) || (ptr == orig),
...
template<int(Allocm)(void**, size_t*, size_t, int), int(Rallocm)(void**, size_t*, size_t, size_t, int),
         int(Sallocm)(const void*, size_t*, int), int(Dallocm)(void*, int), int(Nallocm)(size_t*, size_t, int)>
...
int main() {
    ...
    static_assert(OUR_MALLOCX_ZERO == MALLOCX_ZERO, "Our MALLOCX_ZERO macro is wrong");
    static_assert(OUR_MALLOCX_ZERO == ALLOCM_ZERO, "Our ALLOCM_ZERO macro is wrong");
    static_assert(OUR_ALLOCM_NO_MOVE == ALLOCM_NO_MOVE, "Our ALLOCM_NO_MOVE macro is wrong");
    static_assert(JEMallocFlags(MALLOCX_ZERO).should_zero(), "Our MALLOCX_ZERO is not the value that we are using");
    static_assert(!JEMallocFlags(~MALLOCX_ZERO).should_zero(), "Our MALLOCX_ZERO is not the value that we are using");
    static_assert(JEMallocFlags(ALLOCM_NO_MOVE).may_not_move(),
    ...
    test_legacy_experimental_apis<our_allocm, our_rallocm, our_sallocm, our_dallocm, our_nallocm>();
