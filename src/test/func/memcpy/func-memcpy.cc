    ...
    for (size_t start = 0; start < size; start++) {
        ...
        for (size_t i = 0; i < sz; ++i) {
            if (dst[i] != static_cast<unsigned char>(i)) {
                fprintf(stderr, "Testing size %zd %hhx == %hhx\n", sz, static_cast<unsigned char>(i), dst[i]);
            }
            EXPECT(dst[i] == (unsigned char)i, "dst[i] == {}, i == {}", size_t(dst[i]), i & 0xff);
        }
...
void check_bounds(size_t size, size_t out_of_bounds) {
    START_TEST("memcpy bounds, size {}, {} bytes out of bounds", size, out_of_bounds);
    ...
    can_longjmp = false;
    EXPECT(bounds_error == (out_of_bounds > 0), "bounds error: {}, out_of_bounds: {}", bounds_error, out_of_bounds);
    my_free(s);
    my_free(d);
}
...
#  if !defined(SNMALLOC_PASS_THROUGH)
    ...
    static constexpr size_t min_class_size = sizeclass_to_size(size_to_sizeclass(MIN_ALLOC_SIZE));

    std::initializer_list<size_t> sizes = {min_class_size, 1024, 2 * 1024 * 1024};
    static_assert(MIN_ALLOC_SIZE < 1024, "Can't detect overflow except at sizeclass boundaries");
    for (auto sz : sizes) {
        // Check in bounds
        check_bounds(sz, 0);
        // Check one byte out
