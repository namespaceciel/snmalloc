...
constexpr size_t rotr_const(size_t x, size_t n) {
    size_t nn = n & (BITS - 1);
    return (x >> nn) | (x << ((static_cast<size_t>(-static_cast<int>(nn))) & (BITS - 1)));
}
...
constexpr SNMALLOC_FAST_PATH size_t align_down(size_t value, size_t alignment) {
...
constexpr size_t to_exp_mant_const(size_t value) {
    ...
    size_t e = bits::BITS - MANTISSA_BITS - LOW_BITS - clz_const(value | LEADING_BIT);
    size_t b = (e == 0) ? 0 : 1;
