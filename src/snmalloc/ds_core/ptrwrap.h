 ...
 */
template<typename T>
SNMALLOC_FAST_PATH_INLINE uintptr_t unsafe_to_uintptr(std::enable_if_t<true, T>* p) {
 ...
 * dimensions, above.
 */
template<dimension::Spatial S, dimension::AddressSpaceControl AS, dimension::Wildness W>
struct bound {
    static constexpr enum dimension::Spatial spatial                           = S;
    static constexpr enum dimension::AddressSpaceControl address_space_control = AS;
    ...
    static_assert(!(W == dimension::Wildness::Wild)
                      || (S == dimension::Spatial::Alloc && AS == dimension::AddressSpaceControl::User),
                  "Wild pointers must be annotated as tightly bounded");
    static_assert((S != dimension::Spatial::Arena)
                      || (W == dimension::Wildness::Tame && AS == dimension::AddressSpaceControl::Full),
 ...
 */
using Arena = bound<dimension::Spatial::Arena, dimension::AddressSpaceControl::Full, dimension::Wildness::Tame>;

/**
 * Internal access to a Chunk of memory.  These flow across the boundary
 * between back- and front-ends, for example.
 */
using Chunk = bound<dimension::Spatial::Chunk, dimension::AddressSpaceControl::Full, dimension::Wildness::Tame>;

/**
 * User access to an entire Chunk.  Used as an ephemeral state when
 * returning a large allocation.  See capptr_chunk_is_alloc.
 */
using ChunkUser = Chunk::with_address_space_control<dimension::AddressSpaceControl::User>;
 ...
 */
using Alloc = AllocFull::with_address_space_control<dimension::AddressSpaceControl::User>;
...
using user_address_control_type = typename B::template with_address_space_control<dimension::AddressSpaceControl::User>;

/**
 * Determine whether BI is a spatial refinement of BO.
 * Chunk and ChunkD are considered eqivalent here.
 */
template<SNMALLOC_CONCEPT(capptr::IsBound) BI, SNMALLOC_CONCEPT(capptr::IsBound) BO>
...
public:
    ...
    constexpr SNMALLOC_FAST_PATH CapPtr()
        : CapPtr(nullptr) {}
...
#endif
    constexpr explicit SNMALLOC_FAST_PATH CapPtr(T* p)
        : unsafe_capptr(p) {}
#ifdef _MSC_VER
#  pragma warning(pop)
#endif
...
public:
    ...
    [[nodiscard]] SNMALLOC_FAST_PATH CapPtr<U, bounds> as_static() const {
        return CapPtr<U, bounds>::unsafe_from(static_cast<U*>(this->unsafe_capptr));
    }
    ...
    [[nodiscard]] SNMALLOC_FAST_PATH CapPtr<U, bounds> as_reinterpret() const {
        return CapPtr<U, bounds>::unsafe_from(reinterpret_cast<U*>(this->unsafe_capptr));
    }
    ...
    SNMALLOC_FAST_PATH T* operator->() const {
        static_assert(bounds::wildness != capptr::dimension::Wildness::Wild, "Trying to dereference a Wild pointer");
 ...
 */
template<typename T>
inline SNMALLOC_FAST_PATH capptr::Alloc<T> capptr_chunk_is_alloc(capptr::ChunkUser<T> p) {
 ...
 */
inline static SNMALLOC_FAST_PATH capptr::AllocWild<void> capptr_from_client(void* p) {
...
inline static SNMALLOC_FAST_PATH CapPtr<T, typename B::template with_wildness<capptr::dimension::Wildness::Wild>>
capptr_rewild(CapPtr<T, B> p) {
    return CapPtr<T, typename B::template with_wildness<capptr::dimension::Wildness::Wild>>::unsafe_from(
        p.unsafe_ptr());
}
...
public:
     ...
     * default to nullptr
     */
    constexpr SNMALLOC_FAST_PATH AtomicCapPtr()
        : AtomicCapPtr(nullptr) {}
    ...
    SNMALLOC_FAST_PATH CapPtr<T, bounds> load(std::memory_order order = std::memory_order_seq_cst) noexcept {
        return CapPtr<T, bounds>::unsafe_from(this->unsafe_capptr.load(order));
    }

    SNMALLOC_FAST_PATH void store(CapPtr<T, bounds> desired,
                                  std::memory_order order = std::memory_order_seq_cst) noexcept {
        this->unsafe_capptr.store(desired.unsafe_ptr(), order);
    }

    SNMALLOC_FAST_PATH CapPtr<T, bounds> exchange(CapPtr<T, bounds> desired,
                                                  std::memory_order order = std::memory_order_seq_cst) noexcept {
        return CapPtr<T, bounds>::unsafe_from(this->unsafe_capptr.exchange(desired.unsafe_ptr(), order));
    }
