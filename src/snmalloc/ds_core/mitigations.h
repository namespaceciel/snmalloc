...
inline SNMALLOC_FAST_PATH void check_client_error(const char* const str, Args... args) {
    //[[clang::musttail]]
    return snmalloc::report_fatal_error(str, args...);
}

template<typename... Args>
inline SNMALLOC_FAST_PATH void check_client_impl(bool test, const char* const str, Args... args) {
...
constexpr mitigation::type full_checks = random_pagemap + random_larger_thresholds + freelist_forward_edge
                                       + freelist_backward_edge + freelist_teardown_validate + random_initial
                                       + random_preserve + metadata_protection + random_extra_slab + reuse_LIFO
                                       + sanity_checks + clear_meta + pal_enforce_access;
...
constexpr mitigation::type mitigations =
     ...
     */
    CHECK_CLIENT ? full_checks - metadata_protection - random_pagemap : no_checks;
    ...
    CHECK_CLIENT ?
                  ...
                  * down on CHERI.
                  */
        full_checks + cheri_checks + clear_meta - freelist_forward_edge - pal_enforce_access
                 :
                 /**
