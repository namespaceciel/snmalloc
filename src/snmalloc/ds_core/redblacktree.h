...
concept RBRepMethods =
    requires(typename Rep::Handle hp, typename Rep::Contents k, bool b) {
        ...
        {
            typename Rep::Handle{
                const_cast<std::remove_const_t<std::remove_reference_t<decltype(Rep::root)>>*>(&Rep::root)}
 ...
 */
template<SNMALLOC_CONCEPT(RBRep) Rep, bool run_checks = DEBUG, bool TRACE = false>
class RBTree {
    ...
    typename std::remove_const_t<std::remove_reference_t<decltype(Rep::root)>> root{Rep::root};
    ...
    int invariant(K curr, K lower = Rep::null, K upper = Rep::null) {
        if constexpr (!run_checks) {
            ...
            if (((lower != Rep::null) && Rep::compare(lower, curr))
                || ((upper != Rep::null) && Rep::compare(curr, upper))) {
                report_fatal_error("Invariant failed: {} is out of bounds {}..{}", Rep::printable(curr),
                                   Rep::printable(lower), Rep::printable(upper));
            }

            if (Rep::is_red(curr) && (Rep::is_red(get_dir(true, curr)) || Rep::is_red(get_dir(false, curr)))) {
                report_fatal_error("Invariant failed: {} is red and has red child", Rep::printable(curr));
            }
            ...
            if (left_inv != right_inv) {
                report_fatal_error("Invariant failed: {} has different black depths", Rep::printable(curr));
            }
        ...
        void print() {
            if constexpr (TRACE) {
                for (size_t i = 0; i < length; i++) {
                    message<1024>("  -> {} @ {} ({})", Rep::printable(K(path[i].node)), path[i].node.printable(),
                                  path[i].dir);
                }
            }
        }
...
public:
    ...
    void print(ChildRef curr, const char* indent = "", size_t depth = 0) {
        if constexpr (TRACE) {
            ...
            message<1024>("{}\\_{}{}{}@{} ({})", indent, colour, Rep::printable((K(curr))), reset, curr.printable(),
                          depth);
    ...
    bool remove_path(RBPath& path) {
        ...
        while (path.curr() != ChildRef(H{&root})) {
            ...
            if (Rep::is_red(parent)) {
                ...
                debug_log("Black sibling and red parent case - done", path, path.parent());
                break;
            }
            // Handle black sibling and niblings and black parent.
            debug_log("Black sibling, niblings and black parent case", path, path.parent());
            Rep::set_red(sibling, true);
            path.pop();
            invariant(path.curr());
            debug_log("Black sibling, niblings and black parent case - done", path, path.curr());
        }
        return true;
    }
    ...
    void insert_path(RBPath& path, K value) {
        ...
        while (path.curr() != get_root()) {
            ...
            if (path.parent_dir() == curr_dir) {
                ...
                debug_log("Insert - double red case 1 - done", path, path.grand_parent());
            } else {
                ...
                debug_log("Insert - double red case 2 - done", path, path.grand_parent());
            }
