...
public:
    ...
    static void* reserve_aligned(size_t size) noexcept {
        ...
        vm_offset_t addr;
        if (vmem_xalloc(kernel_arena, size, align, 0, 0, VMEM_ADDR_MIN, VMEM_ADDR_MAX, M_BESTFIT, &addr)) {
            return nullptr;
        }
        if (state_using) {
            if (kmem_back(kernel_object, addr, size, M_ZERO | M_WAITOK) != KERN_SUCCESS) {
