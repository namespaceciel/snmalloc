...
#    if (NTDDI_VERSION >= NTDDI_WIN10_RS5) && (WINVER >= _WIN32_WINNT_WIN10) && !defined(USE_SYSTEMATIC_TESTING)
...
class PALWindows : public PalTimerDefaultImpl<PALWindows>,
     ...
     * notifications.
     */
    inline static std::atomic<bool> registered_for_notifications;
    inline static HANDLE lowMemoryObject;

    /**
     * List of callbacks for low-memory notification
     */
    inline static PalNotifier low_memory_callbacks;
...
public:
     ...
     */
    static constexpr uint64_t pal_features = LowMemoryNotification | Entropy | Time
...
#  endif
     ...
     */
    static void register_for_low_memory_callback(PalNotificationObject* callback) {
        ...
        if (!registered_for_notifications.exchange(true)) {
            lowMemoryObject = CreateMemoryResourceNotification(LowMemoryResourceNotification);
            HANDLE waitObject;
            RegisterWaitForSingleObject(&waitObject, lowMemoryObject, low_memory, nullptr, INFINITE, WT_EXECUTEDEFAULT);
        }
    ...
    static void notify_using(void* p, size_t size) noexcept {
        SNMALLOC_ASSERT(is_aligned_block<page_size>(p, size) || (zero_mem == NoZero));

        void* r = VirtualAlloc(p, size, MEM_COMMIT, PAGE_READWRITE);

        if (r == nullptr) {
            report_fatal_error("out of memory: {} ({}) could not be committed", p, size);
        }
    }
...
#  ifdef PLATFORM_HAS_VIRTUALALLOC2
    ...
    static void* reserve_aligned(size_t size) noexcept {
        ...
        void* ret = VirtualAlloc2FromApp(nullptr, nullptr, size, flags, PAGE_READWRITE, &param, 1);
        if (ret == nullptr) {
            errno = ENOMEM;
        }
        return ret;
...
#  endif
    ...
    static uint64_t get_entropy64() {
        uint64_t result;
        if (BCryptGenRandom(nullptr, reinterpret_cast<PUCHAR>(&result), sizeof(result), BCRYPT_USE_SYSTEM_PREFERRED_RNG)
            != 0) {
