...
public:
     ...
     */
    static constexpr uint64_t pal_features = AlignedAllocation | PALBSD<OS>::pal_features;

    static SNMALLOC_CONSTINIT_STATIC size_t minimum_alloc_size = aal_supports<StrictProvenance> ? 1 << 24 : 4096;
    ...
    static void* reserve_aligned(size_t size) noexcept {
        ...
        auto prot = state_using || !mitigations(pal_enforce_access) ? PROT_READ | PROT_WRITE : PROT_NONE;

        void* p = mmap(nullptr, size, prot,
                       MAP_PRIVATE | MAP_ANONYMOUS | MAP_ALIGNED(log2align) | OS::extra_mmap_flags(state_using), -1, 0);
