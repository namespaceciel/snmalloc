...
#    if defined(__MAC_OS_X_VERSION_MIN_REQUIRED) && defined(MAC_OS_X_VERSION_14_4)
...
extern "C" int __ulock_wait(uint32_t lock_type, void* addr, uint64_t value, uint32_t);
...
public:
     ...
     */
    static constexpr uint64_t pal_features = AlignedAllocation | LazyCommit | Entropy | Time | WaitOnAddress;
    ...
    static constexpr int anonymous_memory_fd = int(VM_MAKE_TAG(uint32_t(PALAnonID)));

    // Encoded memory tag passed to `mach_vm_map`.
    static constexpr int default_mach_vm_map_flags = int(VM_MAKE_TAG(uint32_t(PALAnonID)));
    ...
    static void* reserve_aligned(size_t size) noexcept {
        ...
        vm_prot_t prot =
            (state_using || !mitigations(pal_enforce_access)) ? VM_PROT_READ | VM_PROT_WRITE : VM_PROT_NONE;
    ...
    static uint64_t get_entropy64() {
        uint64_t result;
        if (CCRandomGenerateBytes(reinterpret_cast<void*>(&result), sizeof(result)) != kCCSuccess) {
...
#  ifdef SNMALLOC_APPLE_HAS_OS_SYNC_WAIT_ON_ADDRESS
            if (os_sync_wait_on_address(&addr, static_cast<uint64_t>(expected), sizeof(T), 0) != -1) {
                errno = errno_backup;
                return;
            }
#  else
            if (__ulock_wait(UL_COMPARE_AND_WAIT | ULF_NO_ERRNO, &addr, static_cast<uint64_t>(expected), 0) != -1) {
                return;
            }
#  endif
        }
