...
public:
    ...
    static_assert(bits::is_pow2(SNMALLOC_PAGESIZE), "Page size must be a power of 2");
    static constexpr size_t page_size = SNMALLOC_PAGESIZE;
#elif defined(PAGESIZE)
    static constexpr size_t page_size = bits::max(Aal::smallest_page_size, static_cast<size_t>(PAGESIZE));
#else
    ...
    static void notify_using(void* p, size_t size) noexcept {
        SNMALLOC_ASSERT(is_aligned_block<OS::page_size>(p, size) || (zero_mem == NoZero));
