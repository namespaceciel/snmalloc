 ...
 */
template<typename PAL = DefaultPal, typename AAL = Aal, typename T, SNMALLOC_CONCEPT(capptr::IsBound) B>
inline static
    typename std::enable_if_t<!aal_supports<StrictProvenance, AAL>, CapPtr<T, capptr::user_address_control_type<B>>>
    capptr_to_user_address_control(CapPtr<T, B> p) {
    return CapPtr<T, capptr::user_address_control_type<B>>::unsafe_from(p.unsafe_ptr());
}
 ...
 */
template<typename PAL, bool page_aligned = false, typename T, SNMALLOC_CONCEPT(capptr::IsBound) B>
static SNMALLOC_FAST_PATH void pal_zero(CapPtr<T, B> p, size_t sz) {
    static_assert(!page_aligned || B::spatial >= capptr::dimension::Spatial::Chunk);
    PAL::template zero<page_aligned>(p.unsafe_ptr(), sz);
}

static_assert(bits::is_pow2(OS_PAGE_SIZE), "OS_PAGE_SIZE must be a power of two");
static_assert(OS_PAGE_SIZE % Aal::smallest_page_size == 0,
