...
class PalList {
    ...
    static_assert(std::is_same<decltype(T::pal_next), std::atomic<T*>>::value, "Required pal_next type.");
...
public:
    ...
    void check(uint64_t time_ms) {
        ...
        timers.apply_all([time_ms](PalTimerObject* curr) {
            if ((curr->last_run == 0) || ((time_ms - curr->last_run) > curr->repeat)) {
