...
public:
     ...
     */
    static constexpr uint64_t pal_features = PALBSD_Aligned::pal_features | CoreDump | WaitOnAddress;
...
#  endif
    ...
    static void wait_on_address(std::atomic<T>& addr, T expected) {
        ...
        while (addr.load(std::memory_order_relaxed) == expected) {
            int ret =
                _umtx_op(&addr, UMTX_OP_WAIT_UINT_PRIVATE, static_cast<unsigned long>(expected), nullptr, nullptr);
    ...
    static void notify_all_on_address(std::atomic<T>& addr) {
        static_assert(sizeof(T) == sizeof(WaitingWord) && alignof(T) == alignof(WaitingWord),
                      "T must be the same size and alignment as WaitingWord");
        _umtx_op(&addr, UMTX_OP_WAKE_PRIVATE, static_cast<unsigned long>(INT_MAX), nullptr, nullptr);
    }
};
} // namespace snmalloc
#endif
