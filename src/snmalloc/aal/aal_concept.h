...
template<typename AAL>
concept IsAAL_capptr_methods = requires(capptr::Chunk<void> auth, capptr::AllocFull<void> ret, size_t sz) {
                                   /**
                                    ...
                                    */
                                   { AAL::capptr_rebound(auth, ret) } noexcept -> ConceptSame<capptr::Chunk<void>>;
...
template<typename AAL>
concept IsAAL = IsAAL_static_members<AAL> && IsAAL_prefetch<AAL> && IsAAL_tick<AAL> && IsAAL_capptr_methods<AAL>;
