 ...
 */
SNMALLOC_FAST_PATH_INLINE uintptr_t pointer_offset(uintptr_t base, size_t diff) {
    return base + diff;
}
...
SNMALLOC_FAST_PATH_INLINE U* pointer_offset(T* base, size_t diff) {
    ...
    return unsafe_from_uintptr<U>(unsafe_to_uintptr<T>(base) + static_cast<uintptr_t>(diff));
}

template<SNMALLOC_CONCEPT(capptr::IsBound) bounds, typename T>
SNMALLOC_FAST_PATH_INLINE CapPtr<void, bounds> pointer_offset(CapPtr<T, bounds> base, size_t diff) {
    return CapPtr<void, bounds>::unsafe_from(pointer_offset(base.unsafe_ptr(), diff));
}
...
template<SNMALLOC_CONCEPT(capptr::IsBound) bounds, typename T>
SNMALLOC_FAST_PATH_INLINE CapPtr<void, bounds> pointer_offset_signed(CapPtr<T, bounds> base, ptrdiff_t diff) {
    return CapPtr<void, bounds>::unsafe_from(pointer_offset_signed(base.unsafe_ptr(), diff));
}
...
SNMALLOC_FAST_PATH_INLINE T* pointer_align_down(void* p) {
    return unsafe_from_uintptr<T>(pointer_align_down<alignment>(unsafe_to_uintptr<void>(p)));
}

template<size_t alignment, typename T, SNMALLOC_CONCEPT(capptr::IsBound) bounds>
SNMALLOC_FAST_PATH_INLINE CapPtr<T, bounds> pointer_align_down(CapPtr<void, bounds> p) {
    return CapPtr<T, bounds>::unsafe_from(pointer_align_down<alignment, T>(p.unsafe_ptr()));
}
...
#else
        return unsafe_from_uintptr<T>(bits::align_up(unsafe_to_uintptr<void>(p), alignment));
#endif
    }
}

template<size_t alignment, typename T = void, SNMALLOC_CONCEPT(capptr::IsBound) bounds>
SNMALLOC_FAST_PATH_INLINE CapPtr<T, bounds> pointer_align_up(CapPtr<void, bounds> p) {
    return CapPtr<T, bounds>::unsafe_from(pointer_align_up<alignment, T>(p.unsafe_ptr()));
}
...
#else
    return unsafe_from_uintptr<T>(bits::align_down(unsafe_to_uintptr<void>(p), alignment));
#endif
}

template<typename T = void, SNMALLOC_CONCEPT(capptr::IsBound) bounds>
SNMALLOC_FAST_PATH_INLINE CapPtr<T, bounds> pointer_align_down(CapPtr<void, bounds> p, size_t alignment) {
    return CapPtr<T, bounds>::unsafe_from(pointer_align_down<T>(p.unsafe_ptr(), alignment));
}
...
template<typename T = void, SNMALLOC_CONCEPT(capptr::IsBound) bounds>
SNMALLOC_FAST_PATH_INLINE CapPtr<T, bounds> pointer_align_up(CapPtr<void, bounds> p, size_t alignment) {
    return CapPtr<T, bounds>::unsafe_from(pointer_align_up<T>(p.unsafe_ptr(), alignment));
}
 ...
 */
SNMALLOC_FAST_PATH_INLINE size_t pointer_diff(const void* base, const void* cursor) {
    SNMALLOC_ASSERT(cursor >= base);
    return static_cast<size_t>(static_cast<const char*>(cursor) - static_cast<const char*>(base));
}

template<typename T = void, typename U = void, SNMALLOC_CONCEPT(capptr::IsBound) Tbounds,
         SNMALLOC_CONCEPT(capptr::IsBound) Ubounds>
SNMALLOC_FAST_PATH_INLINE size_t pointer_diff(CapPtr<T, Tbounds> base, CapPtr<U, Ubounds> cursor) {
 ...
 * across allocations.
 */
SNMALLOC_FAST_PATH_INLINE ptrdiff_t pointer_diff_signed(void* base, void* cursor) {
    return static_cast<ptrdiff_t>(static_cast<char*>(cursor) - static_cast<char*>(base));
}

template<typename T = void, typename U = void, SNMALLOC_CONCEPT(capptr::IsBound) Tbounds,
         SNMALLOC_CONCEPT(capptr::IsBound) Ubounds>
SNMALLOC_FAST_PATH_INLINE ptrdiff_t pointer_diff_signed(CapPtr<T, Tbounds> base, CapPtr<U, Ubounds> cursor) {
