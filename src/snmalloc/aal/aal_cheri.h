...
public:
     ...
     * requirements.
     */
    static constexpr uint64_t aal_features = (Base::aal_features & ~IntegerPointers) | StrictProvenance;
    ...
    static constexpr uint64_t aal_cheri_features =
        ...
        (Base::aal_name == RISCV ? SetBoundsTrapsUntagged | AndPermsTrapsUntagged : 0);
    ...
    static SNMALLOC_FAST_PATH CapPtr<T, BOut> capptr_bound(CapPtr<U, BIn> a, size_t size) noexcept {
        ...
        SNMALLOC_ASSERT_MSG(__builtin_cheri_tag_get(pb), "capptr_bound exactness failed. {} of size {}", a.unsafe_ptr(),
                            size);
    ...
    static SNMALLOC_FAST_PATH CapPtr<T, BOut> capptr_rebound(CapPtr<T, BOut> a, CapPtr<U, BIn> b) noexcept {
        return CapPtr<T, BOut>::unsafe_from(
            static_cast<T*>(__builtin_cheri_address_set(a.unsafe_ptr(), address_cast(b))));
    }
