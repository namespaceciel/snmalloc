...
SNMALLOC_FAST_PATH_INLINE void block_copy(void* dst, const void* src, size_t len) {
 ...
 */
template<size_t Size>
SNMALLOC_FAST_PATH_INLINE void copy_end(void* dst, const void* src, size_t len) {
    copy_one<Size>(pointer_offset(dst, len - Size), pointer_offset(src, len - Size));
}
...
SNMALLOC_FAST_PATH_INLINE bool is_aligned_memcpy(void* dst, const void* src) {
    return (pointer_align_down<Size>(const_cast<void*>(src)) == src) && (pointer_align_down<Size>(dst) == dst);
}
...
SNMALLOC_FAST_PATH_INLINE void small_copy(void* dst, const void* src) {
    ...
    if constexpr (Size != 0) {
        if constexpr (Size >= Word) {
            ...
            small_copy<Size - Word, Word>(pointer_offset(dst, Word), pointer_offset(src, Word));
        } else {
...
SNMALLOC_FAST_PATH_INLINE void unaligned_start(void*& dst, const void*& src, size_t& len) {
...
struct GenericArch {
    ...
    static constexpr size_t LargestRegisterSize = std::max(sizeof(uint64_t), sizeof(void*));

    /**
     * Hook for architecture-specific optimisations.
     */
    static SNMALLOC_FAST_PATH_INLINE void copy(void* dst, const void* src, size_t len) {
...
struct GenericStrictProvenance {
     ...
     */
    static_assert(alignof(void*) == sizeof(void*)); // NOLINT(misc-redundant-expression)
    ...
    static SNMALLOC_FAST_PATH_INLINE void copy(void* dst, const void* src, size_t len) {
         ...
         */
        if (len < sizeof(void*)
                      + (static_cast<size_t>(-static_cast<ptrdiff_t>(address_cast(src))) & (alignof(void*) - 1))) {
         ...
         */
        else if (address_misalignment<alignof(void*)>(address_cast(src))
            ...
            if (sizeof(void*) <= len) {
                ...
                auto dp                  = pointer_align_down<alignof(void*)>(pointer_offset_signed(dep, o));
                auto sp                  = pointer_align_down<alignof(void*)>(pointer_offset_signed(sep, o));
...
#  elif defined(_MSC_VER)
            __movsb(static_cast<unsigned char*>(dst), static_cast<const unsigned char*>(src), len);
#  else
...
using DefaultArch =
    ...
    PPC64Arch
#else
    std::conditional_t<aal_supports<StrictProvenance>, GenericStrictProvenance, GenericArch>
#endif
    ;
...
SNMALLOC_FAST_PATH_INLINE void* memcpy(void* dst, const void* src, size_t len) {
    ...
    if (SNMALLOC_UNLIKELY(!check_bounds<(Checked && ReadsChecked)>(src, len))) {
        return report_fatal_bounds_error(src, len, "memcpy with source out of bounds of heap allocation");
    }
    if (SNMALLOC_UNLIKELY(!check_bounds<Checked>(dst, len))) {
        return report_fatal_bounds_error(dst, len, "memcpy with destination out of bounds of heap allocation");
    }
