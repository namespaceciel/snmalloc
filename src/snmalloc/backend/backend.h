 ...
 */
template<SNMALLOC_CONCEPT(IsPAL) PAL, typename PagemapEntry, typename Pagemap, typename Authmap, typename LocalState>
...
public:
     ...
     */
    template<typename T>
    static capptr::Alloc<void> alloc_meta_data(LocalState* local_state, size_t size) {
        capptr::Arena<void> p;
        ...
        if (local_state != nullptr) {
            ...
            static_assert(GlobalMetaRange::ConcurrencySafe, "Global meta data range needs to be concurrency safe.");
        ...
        return capptr_to_user_address_control(Aal::capptr_bound<void, capptr::bounds::AllocFull>(p, size));
    }
     ...
     */
    static void dealloc_meta_data(LocalState& local_state, capptr::Alloc<void> p, size_t size) {
     ...
     */
    static std::pair<capptr::Chunk<void>, SlabMetadata*> alloc_chunk(LocalState& local_state, size_t size,
                                                                     uintptr_t ras, sizeclass_t sizeclass) {
     ...
     */
    static void dealloc_chunk(LocalState& local_state, SlabMetadata& slab_metadata, capptr::Alloc<void> alloc,
                              size_t size, sizeclass_t sizeclass) {
        /*
        ...
        t.claim_for_backend();
        SNMALLOC_ASSERT_MSG(Pagemap::get_metaentry(address_cast(alloc)).get_slab_metadata() == &slab_metadata,
                            "Slab metadata {} passed for address {} does not match the meta entry "
                            "{} that is used for that address",
                            &slab_metadata, address_cast(alloc),
        ...
        auto meta_size = bits::next_pow2(sizeof(SlabMetadata) + extra_bytes);
        local_state.get_meta_range().dealloc_range(capptr::Arena<void>::unsafe_from(&slab_metadata), meta_size);

        local_state.get_object_range()->dealloc_range(arena, size);
    }

    SNMALLOC_FAST_PATH static capptr::Alloc<void> capptr_rederive_alloc(capptr::Alloc<void> a, size_t objsize) {
        return capptr_to_user_address_control(
            Aal::capptr_bound<void, capptr::bounds::AllocFull>(Authmap::amplify(a), objsize));
    }
