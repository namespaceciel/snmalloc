 ...
 */
template<SNMALLOC_CONCEPT(IsPAL) PAL, typename ClientMetaDataProvider = NoClientMetaDataProvider>
...
private:
    using ConcretePagemap = FlatPagemap<MIN_CHUNK_BITS, PagemapEntry, PAL, true>;

    using Pagemap = BasicPagemap<PAL, ConcretePagemap, PagemapEntry, true>;

    struct Authmap {
        inline static capptr::Arena<void> arena;
...
public:
    ...
    static void init(LocalState* local_state, void* base, size_t length) {
        ...
        auto [heap_base, heap_length] = Pagemap::concretePagemap.init(base, length);
        ...
        range_to_pow_2_blocks<MIN_CHUNK_BITS>(capptr::Arena<void>::unsafe_from(heap_base), heap_length,
    ...
    static SNMALLOC_FAST_PATH CapPtr<T, typename B::template with_wildness<capptr::dimension::Wildness::Tame>>
    capptr_domesticate(LocalState* ls, CapPtr<T, B> p) {
        static_assert(B::wildness == capptr::dimension::Wildness::Wild);

        static const size_t sz = sizeof(std::conditional<std::is_same_v<std::remove_cv<T>, void>, void*, T>);
