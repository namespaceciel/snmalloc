 ...
 * can be used.
 */
template<typename PAL, typename Pagemap, typename Base, size_t MinSizeBits = MinBaseSizeBits<PAL>()>
struct MetaProtectedRangeLocalState : BaseLocalStateConstants {
private:
    // Global range of memory
    using GlobalR = Pipe<Base, LargeBuddyRange<GlobalCacheSizeBits, bits::BITS - 1, Pagemap, MinSizeBits>, LogRange<2>,
                         GlobalRange>;

    static constexpr size_t page_size_bits = bits::next_pow2_bits_const(PAL::page_size);

    static constexpr size_t max_page_chunk_size_bits = bits::max(page_size_bits, MIN_CHUNK_BITS);
    ...
    using CentralObjectRange = Pipe<GlobalR, LargeBuddyRange<GlobalCacheSizeBits, bits::BITS - 1, Pagemap>, LogRange<3>,
                                    GlobalRange, CommitRange<PAL>, StatsRange>;
    ...
    using CentralMetaRange = Pipe<
        GlobalR, SubRange<PAL, SubRangeRatioBits>, // Use SubRange to introduce guard
                                                   // pages.
        LargeBuddyRange<GlobalCacheSizeBits, bits::BITS - 1, Pagemap, page_size_bits>, CommitRange<PAL>,
        // In case of huge pages, we don't want to give each thread its own huge
        // page, so commit in the global range.
        std::conditional_t<(max_page_chunk_size_bits > MIN_CHUNK_BITS),
                           LargeBuddyRange<max_page_chunk_size_bits, max_page_chunk_size_bits, Pagemap, page_size_bits>,
                           NopRange>,
        LogRange<4>, GlobalRange, StatsRange>;

    // Local caching of object range
    using ObjectRange =
        Pipe<CentralObjectRange, LargeBuddyRange<LocalCacheSizeBits, LocalCacheSizeBits, Pagemap, page_size_bits>,
             LogRange<5>>;

    // Local caching of meta-data range
    using MetaRange =
        Pipe<CentralMetaRange, LargeBuddyRange<LocalCacheSizeBits - SubRangeRatioBits, bits::BITS - 1, Pagemap>,
             SmallBuddyRange>;
...
public:
    ...
    using GlobalMetaRange = Pipe<CentralMetaRange, SmallBuddyRange, GlobalRange>;
};
} // namespace snmalloc
