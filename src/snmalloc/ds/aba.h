...
public:
    ...
    static_assert(sizeof(Linked) == sizeof(Independent), "Expecting identical struct sizes in union");
    static_assert(sizeof(Linked) == (2 * sizeof(std::size_t)), "Expecting ABA to be the size of two pointers");
...
#  endif
        return Cmp{
            {independent.ptr.load(std::memory_order_relaxed), independent.aba.load(std::memory_order_relaxed)},
            this
        };
    }
...
#  if defined(_MSC_VER) && defined(SNMALLOC_VA_BITS_64)
            auto result = _InterlockedCompareExchange128((volatile __int64*)parent, (__int64)(old.aba + (uintptr_t)1),
                                                         (__int64)address_cast(value), (__int64*)&old);
#  else
...
#    endif
            ...
            auto result = addr.compare_exchange_weak(old, xchg, std::memory_order_acq_rel, std::memory_order_relaxed);
#  endif
            return result;
        }
