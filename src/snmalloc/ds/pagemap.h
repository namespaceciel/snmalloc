...
public:
    ...
    static constexpr std::enable_if_t<!has_bounds_, size_t> required_size() {
        static_assert(has_bounds_ == has_bounds, "Don't set SFINAE template parameter!");
     ...
     */
    template<bool has_bounds_ = has_bounds>
    std::enable_if_t<has_bounds_, std::pair<void*, size_t>> init(void* b, size_t s) {
        SNMALLOC_ASSERT(!is_initialised());
        ...
        //     s - pagemap_size
        // can underflow.
        static_assert(sizeof(T) < (1 << SHIFT), "Pagemap entry too large relative to granularity");
    ...
    std::enable_if_t<!has_bounds_> init() {
        ...
        if constexpr (randomize_position) {
            ...
            if constexpr (pal_supports<LazyCommit, PAL>) {
                ...
                void* end_page   = pointer_align_up<OS_PAGE_SIZE>(pointer_offset(new_body, REQUIRED_SIZE));
                ...
                // have mapped.
                PAL::notify_using_readonly(start_page, pointer_diff(start_page, end_page));
            }
        } else {
            if constexpr (pal_supports<LazyCommit, PAL>) {
        ...
        PAL::template notify_using<NoZero>(pointer_align_down<OS_PAGE_SIZE>(new_body), OS_PAGE_SIZE);
    ...
    [[nodiscard]] address_t get_address(const T& t) const {
        ...
        SNMALLOC_ASSERT(entry_offset % sizeof(T) == 0 && entry_index < num_entries());
