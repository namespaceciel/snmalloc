...
private:
    ...
    __attribute__((no_sanitize("thread"))) static T* racy_read(std::atomic<T*>& ptr) {
...
public:
    ...
    void push(T* item) {
        static_assert(std::is_same<decltype(T::next), std::atomic<T*>>::value, "T->next must be an std::atomic<T*>");
