...
class CombiningLockNode {
    ...
    static constexpr bool use_wait_on_address =
        pal_supports<PalFeatures::WaitOnAddress, Pal> && SNMALLOC_USE_WAIT_ON_ADDRESS;
    ...
    using WaitingWordType = typename WaitWordTypeSelect<use_wait_on_address<DefaultPal>, DefaultPal>::type;
    ...
    constexpr CombiningLockNode(void (*f)(CombiningLockNode*))
        : f_raw(f) {}
    ...
    static void wake(CombiningLockNode* node, LockStatus message) {
        if constexpr (!use_wait_on_address<Pal>) {
            ...
            if (node->status.exchange(message, std::memory_order_acq_rel) == LockStatus::SLEEPING) {
    ...
    void wait() {
        if constexpr (!use_wait_on_address<Pal>) {
            ...
            if (status.compare_exchange_strong(expected, LockStatus::SLEEPING, std::memory_order_acq_rel)) {
    ...
    SNMALLOC_SLOW_PATH void attach_slow(CombiningLock& lock) {
        ...
        // queue.
        auto curr_c = curr;
        if (lock.last.compare_exchange_strong(curr_c, nullptr, std::memory_order_release, std::memory_order_relaxed)) {
...
class CombiningLockNodeTempl : CombiningLockNode {
    ...
    CombiningLockNodeTempl(CombiningLock& lock, F&& f_)
        : CombiningLockNode([](CombiningLockNode* self) {
              CombiningLockNodeTempl* self_templ = reinterpret_cast<CombiningLockNodeTempl*>(self);
              self_templ->f();
          }),
          f(std::forward<F>(f_)) {
        attach_slow(lock);
