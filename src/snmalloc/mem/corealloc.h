...
class CoreAllocator
    : public std::conditional_t<Config::Options.CoreAllocIsPoolAllocated, Pooled<CoreAllocator<Config>>, Empty> {
     ...
     * allocator
     */
    std::conditional_t<Config::Options.IsQueueInline, RemoteAllocator, RemoteAllocator*> remote_alloc;
     ...
     * externally.
     */
    std::conditional_t<Config::Options.CoreAllocOwnsLocalState, LocalState, LocalState*> backend_state;
    ...
    static SNMALLOC_FAST_PATH void alloc_new_list(capptr::Chunk<void>& bumpptr, BackendSlabMetadata* meta, size_t rsize,
                                                  size_t slab_size, LocalEntropy& entropy) {
        ...
        if constexpr (mitigations(random_initial)) {
            ...
            capptr::Chunk<PreAllocObject> curr = pointer_offset(bumpptr, 0).template as_static<PreAllocObject>();
            curr->next = Aal::capptr_bound<PreAllocObject, capptr::bounds::AllocFull>(curr, rsize);

            uint16_t count = 1;
            for (curr = pointer_offset(curr, rsize).template as_static<PreAllocObject>(); curr.as_void() < slab_end;
                 curr = pointer_offset(curr, rsize).template as_static<PreAllocObject>()) {
                size_t insert_index = entropy.sample(count);
                curr->next          = std::exchange(
                    pointer_offset(bumpptr, insert_index * rsize).template as_static<PreAllocObject>()->next,
                    Aal::capptr_bound<PreAllocObject, capptr::bounds::AllocFull>(curr, rsize));
                count++;
            }
            ...
            auto start_ptr   = pointer_offset(bumpptr, start_index * rsize).template as_static<PreAllocObject>()->next;
            ...
            do {
                ...
                b.add(
                    ...
                    freelist::Object::key_root, key_tweak, entropy);
                curr_ptr = next_ptr;
            ...
            do {
                b.add(
                    // Here begins our treatment of the heap as containing Wild pointers
                    freelist::Object::make<capptr::bounds::AllocWild>(capptr_to_user_address_control(
                        Aal::capptr_bound<void, capptr::bounds::AllocFull>(p.as_void(), rsize))),
                    freelist::Object::key_root, key_tweak, entropy);
    ...
    capptr::Alloc<void> clear_slab(BackendSlabMetadata* meta, smallsizeclass_t sizeclass) {
        auto key_tweak = meta->as_key_tweak();
        freelist::Iter<> fl;
        auto more = meta->free_queue.close(fl, freelist::Object::key_root, key_tweak);
        UNUSED(more);
        auto local_state = backend_state_ptr();
        auto domesticate = [local_state](freelist::QueuePtr p) SNMALLOC_FAST_PATH_LAMBDA {
            return capptr_domesticate<Config>(local_state, p);
        };
        capptr::Alloc<void> p = finish_alloc_no_zero(fl.take(freelist::Object::key_root, domesticate), sizeclass);
        ...
        if constexpr (mitigations(freelist_teardown_validate) || mitigations(clear_meta)) {
            ...
            SNMALLOC_CHECK((count + more) == snmalloc::sizeclass_to_slab_object_count(sizeclass));

            if (more > 0) {
                auto no_more = meta->free_queue.close(fl, freelist::Object::key_root, key_tweak);
            ...
            SNMALLOC_CHECK(count == snmalloc::sizeclass_to_slab_object_count(sizeclass));
        }
        auto start_of_slab = pointer_align_down<void>(p, snmalloc::sizeclass_to_slab_size(sizeclass));

#ifdef SNMALLOC_TRACING
        message<1024>("Slab {} is unused, Object sizeclass {}", start_of_slab.unsafe_ptr(), sizeclass);
#endif
    ...
    SNMALLOC_SLOW_PATH void dealloc_local_slabs(smallsizeclass_t sizeclass) {
        ...
        alloc_classes[sizeclass].available.iterate([this, sizeclass](auto* meta) {
            ...
            if (meta->needed() != 0) {
                if (check_slabs) {
                    meta->free_queue.validate(freelist::Object::key_root, meta->as_key_tweak(), domesticate);
                }
                return;
            }
            ...
            Config::Backend::dealloc_chunk(get_backend_local_state(), *meta, start, sizeclass_to_slab_size(sizeclass),
     ...
     */
    SNMALLOC_SLOW_PATH void dealloc_local_object_meta(const PagemapEntry& entry, BackendSlabMetadata* meta) {
    ...
    SNMALLOC_SLOW_PATH void dealloc_local_object_slow(capptr::Alloc<void> p, const PagemapEntry& entry,
        ...
        if (meta->is_large()) {
            ...
            Config::Backend::dealloc_chunk(get_backend_local_state(), *meta, p, size, entry.get_sizeclass());
    ...
    SNMALLOC_FAST_PATH bool has_messages() {
        auto local_state      = backend_state_ptr();
        auto domesticate_head = [local_state](freelist::QueuePtr p) SNMALLOC_FAST_PATH_LAMBDA {
        ...
        auto domesticate_queue = [local_state](freelist::QueuePtr p) SNMALLOC_FAST_PATH_LAMBDA {
            return capptr_domesticate<Config>(local_state, p);
        };
    ...
    SNMALLOC_SLOW_PATH decltype(auto) handle_message_queue_inner(Action action, Args... args) {
        ...
        auto cb = [this, domesticate, &need_post](capptr::Alloc<RemoteMessage> msg) SNMALLOC_FAST_PATH_LAMBDA {
            auto& entry = Config::Backend::template get_metaentry(snmalloc::address_cast(msg));
        ...
        if constexpr (Config::Options.QueueHeadsAreTame) {
             ...
             * annotating type.
             */
            auto domesticate_first = [](freelist::QueuePtr p) SNMALLOC_FAST_PATH_LAMBDA {
                return freelist::HeadPtr::unsafe_from(p.unsafe_ptr());
            };
    ...
    void handle_dealloc_remote(const PagemapEntry& entry, capptr::Alloc<RemoteMessage> msg, bool& need_post,
        ...
        if (SNMALLOC_LIKELY(entry.get_remote() == public_state())) {
            ...
            auto unreturned = dealloc_local_objects_fast(msg, entry, meta, entropy, domesticate);
        ...
        auto nelem = RemoteMessage::template ring_size<Config>(msg, freelist::Object::key_root,
                                                               entry.get_slab_metadata()->as_key_tweak(), domesticate);
        if (!need_post && !attached_cache->remote_dealloc_cache.reserve_space(entry, nelem)) {
            need_post = true;
        }
        attached_cache->remote_dealloc_cache.template forward<sizeof(CoreAllocator)>(entry.get_remote()->trunc_id(),
                                                                                     msg);
    }
    ...
    void init(Range<capptr::bounds::Alloc>& spare) {
        ...
        if (spare.length != 0) {
             ...
             */
            Config::Backend::dealloc_meta_data(get_backend_local_state(), spare.base, spare.length);
        }
...
public:
     ...
     * reserved as meta-data, but is not required by this CoreAllocator.
     */
    template<typename Config_ = Config, typename = std::enable_if_t<Config_::Options.CoreAllocOwnsLocalState>>
     ...
     */
    template<typename Config_ = Config, typename = std::enable_if_t<!Config_::Options.CoreAllocOwnsLocalState>>
    CoreAllocator(Range<capptr::bounds::Alloc>& spare, LocalCache<Config_>* cache, LocalState* backend = nullptr)
    ...
    SNMALLOC_FAST_PATH bool post() {
        ...
        bool sent_something = attached_cache->remote_dealloc_cache.template post<sizeof(CoreAllocator)>(
    ...
    SNMALLOC_FAST_PATH void dealloc_local_object(CapPtr<void, capptr::bounds::Alloc> p,
    ...
    SNMALLOC_FAST_PATH void dealloc_local_object(CapPtr<void, capptr::bounds::Alloc> p) {
        ...
        // pointers
        dealloc_local_object(p, Config::Backend::template get_metaentry(snmalloc::address_cast(p)));
    }

    SNMALLOC_FAST_PATH static bool dealloc_local_object_fast(CapPtr<void, capptr::bounds::Alloc> p,
                                                             const PagemapEntry& entry, BackendSlabMetadata* meta,
                                                             LocalEntropy& entropy) {
        SNMALLOC_ASSERT(!meta->is_unused());

        snmalloc_check_client(mitigations(sanity_checks), is_start_of_object(entry.get_sizeclass(), address_cast(p)),
                              "Not deallocating start of an object");

        auto cp = p.as_static<freelist::Object::T<>>();

        // Update the head and the next pointer in the free list.
        meta->free_queue.add(cp, freelist::Object::key_root, meta->as_key_tweak(), entropy);

        return SNMALLOC_LIKELY(!meta->return_object());
    }

    template<typename Domesticator>
    SNMALLOC_FAST_PATH static auto dealloc_local_objects_fast(capptr::Alloc<RemoteMessage> msg,
                                                              const PagemapEntry& entry, BackendSlabMetadata* meta,
                                                              LocalEntropy& entropy, Domesticator domesticate) {
        SNMALLOC_ASSERT(!meta->is_unused());

        snmalloc_check_client(mitigations(sanity_checks), is_start_of_object(entry.get_sizeclass(), address_cast(msg)),
                              "Not deallocating start of an object");

        size_t objsize = sizeclass_full_to_size(entry.get_sizeclass());

        auto [curr, length] = RemoteMessage::template open_free_ring<Config>(msg, objsize, freelist::Object::key_root,
                                                                             meta->as_key_tweak(), domesticate);

        // Update the head and the next pointer in the free list.
        meta->free_queue.append_segment(curr, msg.template as_reinterpret<freelist::Object::T<>>(), length,
                                        freelist::Object::key_root, meta->as_key_tweak(), entropy);

        return meta->return_objects(length);
    }

    template<ZeroMem zero_mem>
    SNMALLOC_SLOW_PATH capptr::Alloc<void> small_alloc(smallsizeclass_t sizeclass, freelist::Iter<>& fast_free_list) {
        ...
        if (SNMALLOC_LIKELY(alloc_classes[sizeclass].length > 0)) {
            ...
            auto [p, still_active] =
                BackendSlabMetadata::alloc_free_list(domesticate, meta, fast_free_list, entropy, sizeclass);
    ...
    template<ZeroMem zero_mem>
    SNMALLOC_SLOW_PATH capptr::Alloc<void> small_alloc_slow(smallsizeclass_t sizeclass,
        ...
        auto [slab, meta] =
            Config::Backend::alloc_chunk(get_backend_local_state(), slab_size,
                                         PagemapEntry::encode(public_state(), sizeclass_t::from_small_class(sizeclass)),
                                         sizeclass_t::from_small_class(sizeclass));
        ...
        meta->initialise(sizeclass, address_cast(slab), freelist::Object::key_root);
    ...
    bool flush(bool destroy_queue = false) {
        ...
        if (destroy_queue) {
            auto cb = [this, domesticate](capptr::Alloc<RemoteMessage> m) {
                ...
                const PagemapEntry& entry = Config::Backend::get_metaentry(snmalloc::address_cast(m));
        ...
        auto posted =
            attached_cache->template flush<sizeof(CoreAllocator)>(backend_state_ptr(), [&](capptr::Alloc<void> p) {
                dealloc_local_object(p);
            });

        // We may now have unused slabs, return to the global allocator.
        for (smallsizeclass_t sizeclass = 0; sizeclass < NUM_SMALL_SIZECLASSES; sizeclass++) {
            dealloc_local_slabs<true>(sizeclass);
        }

        laden.iterate([domesticate](BackendSlabMetadata* meta) SNMALLOC_FAST_PATH_LAMBDA {
            if (!meta->is_large()) {
                meta->free_queue.validate(freelist::Object::key_root, meta->as_key_tweak(), domesticate);
            }
        });
    ...
    bool debug_is_empty_impl(bool* result) {
        auto error = [&result](auto slab_metadata) {
            auto slab_interior        = slab_metadata->get_slab_interior(freelist::Object::key_root);
            const PagemapEntry& entry = Config::Backend::get_metaentry(slab_interior);
            ...
            if (result != nullptr) {
                *result = false;
                ...
                report_fatal_error("debug_is_empty: found non-empty allocator: size={} on "
                                   "slab_start {} meta {} entry {}",
                                   sizeclass_full_to_size(size_class), slab_start, address_cast(slab_metadata),
                                   address_cast(&entry));
            }
        };
...
public:
    static capptr::Alloc<CA> make() {
        ...
        auto raw = Config::Backend::template alloc_meta_data<CA>(nullptr, request_size);
 ...
 */
template<typename Config>
using AllocPool = Pool<CoreAllocator<Config>, ConstructCoreAlloc<Config>, Config::pool>;
} // namespace snmalloc
