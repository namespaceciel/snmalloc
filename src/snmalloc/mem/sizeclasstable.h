...
static constexpr smallsizeclass_t size_to_sizeclass_const(size_t size) {
    ...
    auto sc = static_cast<smallsizeclass_t>(bits::to_exp_mant_const<INTERMEDIATE_BITS, MIN_ALLOC_STEP_BITS>(size));
...
constexpr size_t NUM_SMALL_SIZECLASSES = size_to_sizeclass_const(MAX_SMALL_SIZECLASS_SIZE);
...
// sizeclass.
constexpr size_t TAG_SIZECLASS_BITS = bits::max<size_t>(bits::next_pow2_bits_const(NUM_SMALL_SIZECLASSES + 1),
                                                        bits::next_pow2_bits_const(NUM_LARGE_CLASSES + 1));

// Number of bits required to represent a tagged sizeclass that can be
// either small or large.
constexpr size_t SIZECLASS_REP_SIZE = bits::one_at_bit(TAG_SIZECLASS_BITS + 1);
...
class sizeclass_t {
    ...
    constexpr sizeclass_t(size_t value)
        : value(value) {}
...
constexpr SNMALLOC_FAST_PATH static size_t aligned_size(size_t alignment, size_t size) {
...
struct SizeClassTable {
    ...
    [[nodiscard]] constexpr sizeclass_data_fast fast_small(smallsizeclass_t sc) const {
        return fast_[sizeclass_t::from_small_class(sc).raw()];
    }
    ...
    constexpr SizeClassTable() {
        ...
        for (sizeclass_compress_t sizeclass = 0; sizeclass < NUM_SMALL_SIZECLASSES; sizeclass++) {
            auto& meta = fast_small(sizeclass);

            size_t rsize     = bits::from_exp_mant<INTERMEDIATE_BITS, MIN_ALLOC_STEP_BITS>(sizeclass);
            meta.size        = rsize;
            size_t slab_bits = bits::max(bits::next_pow2_bits_const(MIN_OBJECT_COUNT * rsize), MIN_CHUNK_BITS);

            meta.slab_mask = bits::mask_bits(slab_bits);

            auto& meta_slow    = slow(sizeclass_t::from_small_class(sizeclass));
            meta_slow.capacity = static_cast<uint16_t>((meta.slab_mask + 1) / rsize);
...
constexpr uint16_t threshold_for_waking_slab(smallsizeclass_t sizeclass) {
    return sizeclass_metadata.slow(sizeclass_t::from_small_class(sizeclass)).waking;
}
 ...
 */
constexpr size_t metaentry_chunk_sizeclass_to_slab_sizeclass(chunksizeclass_t sizeclass) {
    return sizeclass - MIN_CHUNK_BITS;
}

constexpr uint16_t sizeclass_to_slab_object_count(smallsizeclass_t sizeclass) {
    return sizeclass_metadata.slow(sizeclass_t::from_small_class(sizeclass)).capacity;
}
...
constexpr size_t sizeclass_lookup_size = sizeclass_lookup_index(MAX_SMALL_SIZECLASS_SIZE);
...
struct SizeClassLookup {
    ...
    constexpr SizeClassLookup() {
        constexpr sizeclass_compress_t minimum_class =
            static_cast<sizeclass_compress_t>(size_to_sizeclass_const(MIN_ALLOC_SIZE));
        ...
        for (; sizeclass < minimum_class; sizeclass++) {
            for (; curr <= sizeclass_metadata.fast_small(sizeclass).size; curr += MIN_ALLOC_STEP_SIZE) {
 ...
 */
inline static sizeclass_t size_to_sizeclass_full(size_t size) {
