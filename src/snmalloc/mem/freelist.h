 ...
 */
inline static address_t signed_prev(address_t curr, address_t next, const FreeListKey& key, address_t tweak) {
    auto c = curr;
    auto n = next;
...
namespace freelist {
template<bool RANDOM, bool TRACK_LENGTH = RANDOM, SNMALLOC_CONCEPT(capptr::IsBound) BView = capptr::bounds::Alloc,
...
public:
    ...
    template<SNMALLOC_CONCEPT(capptr::IsBound) BQueue = capptr::bounds::AllocWild>
    class T;
     ...
     */
    template<SNMALLOC_CONCEPT(capptr::IsBound) BView, SNMALLOC_CONCEPT(capptr::IsBound) BQueue>
    ...
    template<SNMALLOC_CONCEPT(capptr::IsBound) BQueue>
    class T {
        template<bool, bool, SNMALLOC_CONCEPT(capptr::IsBound), SNMALLOC_CONCEPT(capptr::IsBound)>
        ...
        public:
            ...
            void check_prev(address_t signed_prev) {
                snmalloc_check_client(mitigations(freelist_backward_edge), signed_prev == prev_encoded,
        ...
        std::conditional_t<mitigations(freelist_backward_edge), Prev, Empty> prev{};

    public:
        constexpr T()
            : next_object() {}

        template<SNMALLOC_CONCEPT(capptr::IsBound)
                     BView = typename BQueue::template with_wildness<capptr::dimension::Wildness::Tame>,
                 typename Domesticator>
        BHeadPtr<BView, BQueue> atomic_read_next(const FreeListKey& key, address_t key_tweak,
                                                 Domesticator domesticate) {
            auto n_wild = Object::decode_next(address_cast(&this->next_object),
                                              this->atomic_next_object.load(std::memory_order_acquire), key, key_tweak);
            auto n_tame = domesticate(n_wild);
            if constexpr (mitigations(freelist_backward_edge)) {
                if (n_tame != nullptr) {
                    n_tame->prev.check_prev(signed_prev(address_cast(this), address_cast(n_tame), key, key_tweak));
                }
            } else {
                UNUSED(key_tweak);
            }
        ...
        template<SNMALLOC_CONCEPT(capptr::IsBound)
                     BView = typename BQueue::template with_wildness<capptr::dimension::Wildness::Tame>,
                 typename Domesticator>
        BHeadPtr<BView, BQueue> read_next(const FreeListKey& key, address_t key_tweak, Domesticator domesticate) {
            return domesticate(
                Object::decode_next(address_cast(&this->next_object), this->next_object, key, key_tweak));
        }
    ...
    template<SNMALLOC_CONCEPT(capptr::IsBound) BQueue, SNMALLOC_CONCEPT(capptr::IsBound) BView>
    static BHeadPtr<BView, BQueue> make(CapPtr<void, BView> p) {
        return CapPtr<Object::T<BQueue>, BView>::unsafe_from(new (p.unsafe_ptr()) Object::T());
    }
     ...
     */
    template<SNMALLOC_CONCEPT(capptr::IsBound) BQueue>
    static Object::T<BQueue>* from_next_ptr(CapPtr<Object::T<BQueue>, BQueue>* ptr) {
     ...
     */
    template<SNMALLOC_CONCEPT(capptr::IsBound) BQueue>
    inline static Object::T<BQueue>* code_next(address_t curr, Object::T<BQueue>* next, const FreeListKey& key,
                                               address_t key_tweak) {
        ...
        if constexpr (mitigations(freelist_forward_edge) && !aal_supports<StrictProvenance>) {
            return unsafe_from_uintptr<Object::T<BQueue>>(unsafe_to_uintptr<Object::T<BQueue>>(next) ^ key.key_next
                                                          ^ key_tweak);
        } else {
            UNUSED(key);
            UNUSED(key_tweak);
     ...
     */
    template<SNMALLOC_CONCEPT(capptr::IsBound) BView, SNMALLOC_CONCEPT(capptr::IsBound) BQueue>
    inline static BQueuePtr<BQueue> encode_next(address_t curr, BHeadPtr<BView, BQueue> next, const FreeListKey& key,
                                                address_t key_tweak) {
        return BQueuePtr<BQueue>::unsafe_from(code_next(curr, next.unsafe_ptr(), key, key_tweak));
    }
     ...
     */
    template<SNMALLOC_CONCEPT(capptr::IsBound) BView, SNMALLOC_CONCEPT(capptr::IsBound) BQueue>
    inline static BHeadPtr<BView, BQueue> decode_next(address_t curr, BHeadPtr<BView, BQueue> next,
                                                      const FreeListKey& key, address_t key_tweak) {
        return BHeadPtr<BView, BQueue>::unsafe_from(code_next(curr, next.unsafe_ptr(), key, key_tweak));
    }
    ...
    static void assert_view_queue_bounds() {
        ...
        static_assert(std::is_same_v<typename BQueue::template with_wildness<capptr::dimension::Wildness::Tame>, BView>,
                      "Free Object Queue bounds must match View bounds (but may be Wild)");
    }

    template<SNMALLOC_CONCEPT(capptr::IsBound) BView, SNMALLOC_CONCEPT(capptr::IsBound) BQueue>
    static void store_nextish(BQueuePtr<BQueue>* curr, BHeadPtr<BView, BQueue> next, const FreeListKey& key,
                              address_t key_tweak, BHeadPtr<BView, BQueue> next_value) {
        assert_view_queue_bounds<BView, BQueue>();

        if constexpr (mitigations(freelist_backward_edge)) {
            next->prev.set_prev(signed_prev(address_cast(curr), address_cast(next), key, key_tweak));
        } else {
            UNUSED(next);
            UNUSED(key);
            UNUSED(key_tweak);
     ...
     */
    template<SNMALLOC_CONCEPT(capptr::IsBound) BView, SNMALLOC_CONCEPT(capptr::IsBound) BQueue>
    static BQueuePtr<BQueue>* store_next(BQueuePtr<BQueue>* curr, BHeadPtr<BView, BQueue> next, const FreeListKey& key,
                                         address_t key_tweak) {
    ...
    template<SNMALLOC_CONCEPT(capptr::IsBound) BQueue>
    static void store_null(BQueuePtr<BQueue>* curr, const FreeListKey& key, address_t key_tweak) {
        *curr = encode_next(address_cast(curr), BQueuePtr<BQueue>(nullptr), key, key_tweak);
    }
     ...
     */
    template<SNMALLOC_CONCEPT(capptr::IsBound) BView, SNMALLOC_CONCEPT(capptr::IsBound) BQueue>
    static void atomic_store_next(BHeadPtr<BView, BQueue> curr, BHeadPtr<BView, BQueue> next, const FreeListKey& key,
                                  address_t key_tweak) {
        static_assert(BView::wildness == capptr::dimension::Wildness::Tame);
        ...
        curr->atomic_next_object.store(encode_next(address_cast(&curr->next_object), next, key, key_tweak),
                                       std::memory_order_release);
    }

    template<SNMALLOC_CONCEPT(capptr::IsBound) BView, SNMALLOC_CONCEPT(capptr::IsBound) BQueue>
    static void atomic_store_null(BHeadPtr<BView, BQueue> curr, const FreeListKey& key, address_t key_tweak) {
        static_assert(BView::wildness == capptr::dimension::Wildness::Tame);

        curr->atomic_next_object.store(
            encode_next(address_cast(&curr->next_object), BQueuePtr<BQueue>(nullptr), key, key_tweak),
 ...
 */
using HeadPtr = Object::BHeadPtr<capptr::bounds::Alloc, capptr::bounds::AllocWild>;

/**
 * Like HeadPtr, but atomic
 */
using AtomicHeadPtr = Object::BAtomicHeadPtr<capptr::bounds::Alloc, capptr::bounds::AllocWild>;
...
using IterBase = std::conditional_t<mitigations(freelist_backward_edge), Prev, NoPrev>;
 ...
 */
template<SNMALLOC_CONCEPT(capptr::IsBound) BView  = capptr::bounds::Alloc,
         SNMALLOC_CONCEPT(capptr::IsBound) BQueue = capptr::bounds::AllocWild>
class Iter : IterBase {
    ...
    SNMALLOC_NO_UNIQUE_ADDRESS
    std::conditional_t<mitigations(freelist_forward_edge) || mitigations(freelist_backward_edge), KeyTweak, NoKeyTweak>
        key_tweak;

public:
    constexpr Iter(Object::BHeadPtr<BView, BQueue> head, address_t prev_value, address_t kt)
    ...
    Object::BHeadPtr<BView, BQueue> take(const FreeListKey& key, Domesticator domesticate) {
        auto c    = curr;
        ...
        if constexpr (mitigations(freelist_backward_edge)) {
            auto p = replace(signed_prev(address_cast(c), address_cast(next), key, key_tweak.get()));
            c->check_prev(p);
        } else {
            UNUSED(key);
        }
 ...
 * randomisation.
 */
template<bool RANDOM, bool TRACK_LENGTH, SNMALLOC_CONCEPT(capptr::IsBound) BView,
...
class Builder {
    ...
    [[nodiscard]] Object::BHeadPtr<BView, BQueue> cast_head(uint32_t ix) const {
        return Object::BHeadPtr<BView, BQueue>::unsafe_from(static_cast<Object::T<BQueue>*>(head[ix]));
    }
...
public:
     ...
     */
    void add(Object::BHeadPtr<BView, BQueue> n, const FreeListKey& key, address_t key_tweak, LocalEntropy& entropy) {
        uint32_t index;
        if constexpr (RANDOM) {
    ...
    std::enable_if_t<!RANDOM_> add(Object::BHeadPtr<BView, BQueue> n, const FreeListKey& key, address_t key_tweak) {
     ...
     */
    SNMALLOC_FAST_PATH void terminate_list(uint32_t index, const FreeListKey& key, address_t key_tweak) {
     ...
     * encoded.
     */
    [[nodiscard]] Object::BHeadPtr<BView, BQueue> read_head(uint32_t index, const FreeListKey& key,
                                                            address_t key_tweak) const {
        return Object::decode_next(address_cast(&head[index]), cast_head(index), key, key_tweak);
    }

    address_t get_fake_signed_prev(uint32_t index, const FreeListKey& key, address_t key_tweak) {
        return signed_prev(address_cast(&head[index]), address_cast(read_head(index, key, key_tweak)), key, key_tweak);
    }
     ...
     * builder.
     */
    SNMALLOC_FAST_PATH uint16_t close(Iter<BView, BQueue>& fl, const FreeListKey& key, address_t key_tweak) {
        uint32_t i;
        if constexpr (RANDOM) {
        ...
        fl = {read_head(i, key, key_tweak), get_fake_signed_prev(i, key, key_tweak), key_tweak};
     ...
     */
    constexpr void init(address_t slab, const FreeListKey& key, address_t key_tweak) {
        for (size_t i = 0; i < LENGTH; i++) {
            end[i] = &head[i];
            if constexpr (TRACK_LENGTH) {
                length[i] = 0;
            ...
            head[i] = Object::code_next(address_cast(&head[i]), useless_ptr_from_addr<Object::T<BQueue>>(slab), key,
                                        key_tweak);
        }
    }
    ...
    template<bool RANDOM_ = RANDOM>
    std::enable_if_t<!RANDOM_, std::pair<Object::BHeadPtr<BView, BQueue>, Object::BHeadPtr<BView, BQueue>>>
    extract_segment(const FreeListKey& key, address_t key_tweak) {
        ...
        auto last = Object::BHeadPtr<BView, BQueue>::unsafe_from(Object::from_next_ptr(cast_end(0)));
     ...
     */
    void append_segment(Object::BHeadPtr<BView, BQueue> first, Object::BHeadPtr<BView, BQueue> last, uint16_t size,
                        const FreeListKey& key, address_t key_tweak, LocalEntropy& entropy) {
        uint32_t index;
        if constexpr (RANDOM) {
            index = entropy.next_bit();
        } else {
    ...
    template<typename Domesticator>
    SNMALLOC_FAST_PATH void validate(const FreeListKey& key, address_t key_tweak, Domesticator domesticate) {
        if constexpr (mitigations(freelist_teardown_validate)) {
            for (uint32_t i = 0; i < LENGTH; i++) {
                ...
                while (true) {
                    ...
                    prev      = signed_prev(address_cast(curr), address_cast(next), key, key_tweak);
                    curr      = next;
                }
