...
namespace snmalloc {
inline static SNMALLOC_FAST_PATH capptr::Alloc<void> finish_alloc_no_zero(freelist::HeadPtr p,
                                                                          smallsizeclass_t sizeclass) {
    SNMALLOC_ASSERT(is_start_of_object(sizeclass_t::from_small_class(sizeclass), address_cast(p)));
    UNUSED(sizeclass);
...
inline static SNMALLOC_FAST_PATH capptr::Alloc<void> finish_alloc(freelist::HeadPtr p, smallsizeclass_t sizeclass) {
    ...
    bool flush(typename Config::LocalState* local_state, DeallocFun dealloc) {
        ...
        for (size_t i = 0; i < NUM_SMALL_SIZECLASSES; i++) {
            ...
            while (!small_fast_free_lists[i].empty()) {
                ...
                SNMALLOC_ASSERT(is_start_of_object(sizeclass_t::from_small_class(i), address_cast(p)));
                dealloc(p.as_void());
            }
        }

        return remote_dealloc_cache.template post<allocator_size>(local_state, remote_allocator->trunc_id());
    }

    template<ZeroMem zero_mem, typename Slowpath, typename Domesticator>
    SNMALLOC_FAST_PATH capptr::Alloc<void> alloc(Domesticator domesticate, size_t size, Slowpath slowpath) {
        auto& key                  = freelist::Object::key_root;
