...
class BatchedRemoteMessage {
    ...
    static_assert(sizeof(free_ring.next_object) >= sizeof(void*),
                  "BatchedRemoteMessage bitpacking needs sizeof(void*) in next_object");

public:
    static auto emplace_in_alloc(capptr::Alloc<void> alloc) {
        return CapPtr<BatchedRemoteMessage, capptr::bounds::Alloc>::unsafe_from(new (alloc.unsafe_ptr())
                                                                                    BatchedRemoteMessage());
    }

    static auto mk_from_freelist_builder(freelist::Builder<false, true>& flb, const FreeListKey& key,
                                         address_t key_tweak) {
        ...
        auto self            = CapPtr<BatchedRemoteMessage, capptr::bounds::Alloc>::unsafe_from(new (last.unsafe_ptr())
                                                                                                    BatchedRemoteMessage());
        ...
        auto n = freelist::HeadPtr::unsafe_from(unsafe_from_uintptr<freelist::Object::T<>>(
    ...
    static freelist::HeadPtr to_message_link(capptr::Alloc<BatchedRemoteMessage> m) {
        return pointer_offset(m, offsetof(BatchedRemoteMessage, message_link)).as_reinterpret<freelist::Object::T<>>();
    }

    static capptr::Alloc<BatchedRemoteMessage> from_message_link(freelist::HeadPtr chainPtr) {
        return pointer_offset_signed(chainPtr, -static_cast<ptrdiff_t>(offsetof(BatchedRemoteMessage, message_link)))
    ...
    open_free_ring(capptr::Alloc<BatchedRemoteMessage> m, size_t objsize, const FreeListKey& key, address_t key_tweak,
                   Domesticator_queue domesticate) {
        uintptr_t encoded = m->free_ring.read_next(key, key_tweak, domesticate).unsafe_uintptr();

        uint16_t decoded_size = static_cast<uint16_t>(encoded) & bits::mask_bits(MAX_CAPACITY_BITS);
        ...
        auto next = domesticate(
            capptr_rewild(Config::Backend::capptr_rederive_alloc(
                              pointer_offset_signed(m, static_cast<ptrdiff_t>(encoded) >> MAX_CAPACITY_BITS), objsize))
                .template as_static<freelist::Object::T<>>());

        if constexpr (mitigations(freelist_backward_edge)) {
            next->check_prev(signed_prev(address_cast(m), address_cast(next), key, key_tweak));
        } else {
            UNUSED(key);
            UNUSED(key_tweak);
        }
    ...
    template<SNMALLOC_CONCEPT(IsConfigLazy) Config, typename Domesticator_queue>
    static uint16_t ring_size(capptr::Alloc<BatchedRemoteMessage> m, const FreeListKey& key, address_t key_tweak,
                              Domesticator_queue domesticate) {
        uintptr_t encoded = m->free_ring.read_next(key, key_tweak, domesticate).unsafe_uintptr();
        ...
        if constexpr (mitigations(freelist_backward_edge)) {
            ...
            auto next = domesticate(
                capptr_rewild(Config::Backend::capptr_rederive_alloc(
                                  pointer_offset_signed(m, static_cast<ptrdiff_t>(encoded) >> MAX_CAPACITY_BITS),
...
class BatchedRemoteMessageAssertions {
    static_assert((DEALLOC_BATCH_RINGS == 0) || (sizeof(BatchedRemoteMessage) <= MIN_ALLOC_SIZE));
    static_assert(offsetof(BatchedRemoteMessage, free_ring) == 0);

    static_assert((DEALLOC_BATCH_RINGS == 0) || (MAX_SLAB_SPAN_BITS + MAX_CAPACITY_BITS < 8 * sizeof(void*)),
...
public:
    static auto emplace_in_alloc(capptr::Alloc<void> alloc) {
        return CapPtr<SingletonRemoteMessage, capptr::bounds::Alloc>::unsafe_from(new (alloc.unsafe_ptr())
                                                                                      SingletonRemoteMessage());
    }

    static freelist::HeadPtr to_message_link(capptr::Alloc<SingletonRemoteMessage> m) {
        return pointer_offset(m, offsetof(SingletonRemoteMessage, message_link))
            .as_reinterpret<freelist::Object::T<>>();
    }

    static capptr::Alloc<SingletonRemoteMessage> from_message_link(freelist::HeadPtr chainPtr) {
        return pointer_offset_signed(chainPtr, -static_cast<ptrdiff_t>(offsetof(SingletonRemoteMessage, message_link)))
            .as_reinterpret<SingletonRemoteMessage>();
    }

    template<SNMALLOC_CONCEPT(IsConfigLazy) Config, typename Domesticator_queue>
    SNMALLOC_FAST_PATH static std::pair<freelist::HeadPtr, uint16_t>
    open_free_ring(capptr::Alloc<SingletonRemoteMessage> m, size_t, const FreeListKey&, address_t, Domesticator_queue) {
        return {m.as_reinterpret<freelist::Object::T<>>(), static_cast<uint16_t>(1)};
    }

    template<SNMALLOC_CONCEPT(IsConfigLazy) Config, typename Domesticator_queue>
    static uint16_t ring_size(capptr::Alloc<SingletonRemoteMessage>, const FreeListKey&, address_t,
                              Domesticator_queue) {
        return 1;
    }
};
...
class SingletonRemoteMessageAssertions {
    ...
    static_assert(sizeof(SingletonRemoteMessage) == sizeof(freelist::Object::T<>));
    static_assert(offsetof(SingletonRemoteMessage, message_link) == 0);
};

using RemoteMessage = std::conditional_t<(DEALLOC_BATCH_RINGS > 0), BatchedRemoteMessage, SingletonRemoteMessage>;
...
struct RemoteAllocator {
    ...
    inline bool can_dequeue(Domesticator_head domesticate_head, Domesticator_queue domesticate_queue) {
    ...
    void enqueue(capptr::Alloc<RemoteMessage> first, capptr::Alloc<RemoteMessage> last,
                 Domesticator_head domesticate_head) {
        list.enqueue(RemoteMessage::to_message_link(first), RemoteMessage::to_message_link(last), domesticate_head);
    }
     ...
     */
    template<typename Domesticator_head, typename Domesticator_queue, typename Cb>
    void dequeue(Domesticator_head domesticate_head, Domesticator_queue domesticate_queue, Cb cb) {
        auto cbwrap = [cb](freelist::HeadPtr p) SNMALLOC_FAST_PATH_LAMBDA {
