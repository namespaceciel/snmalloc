 ...
 */
static constexpr size_t REMOTE_MIN_ALIGN = bits::max<size_t>(CACHELINE_SIZE, SIZECLASS_REP_SIZE) << 1;
...
protected:
     ...
     * allocator entries.
     */
    static constexpr size_t REMOTE_WITH_BACKEND_MARKER_ALIGN = MetaEntryBase::REMOTE_BACKEND_MARKER;
    static_assert((REMOTE_MIN_ALIGN >> 1) == MetaEntryBase::REMOTE_BACKEND_MARKER);
     ...
     */
    static constexpr address_t BACKEND_RESERVED_MASK = (REMOTE_BACKEND_MARKER << 1) - 1;
...
public:
    ...
    [[nodiscard]] bool is_backend_owned() const {
        return (REMOTE_BACKEND_MARKER & remote_and_sizeclass) == REMOTE_BACKEND_MARKER;
    }
    ...
    [[nodiscard]] bool is_unowned() const {
        return ((meta == 0) || (meta == META_BOUNDARY_BIT)) && (remote_and_sizeclass == 0);
    }

    /**
     * Encode the remote and the sizeclass.
     */
    [[nodiscard]] static SNMALLOC_FAST_PATH uintptr_t encode(RemoteAllocator* remote, sizeclass_t sizeclass) {
        /* remote might be nullptr; cast to uintptr_t before offsetting */
        return pointer_offset(reinterpret_cast<uintptr_t>(remote), sizeclass.raw());
    }
    ...
    MetaEntryBase& operator=(const MetaEntryBase& other) {
        ...
        meta                 = (other.meta & ~META_BOUNDARY_BIT) | address_cast(meta & META_BOUNDARY_BIT);
    ...
    [[nodiscard]] SNMALLOC_FAST_PATH RemoteAllocator* get_remote() const {
        ...
        return reinterpret_cast<RemoteAllocator*>(
            pointer_align_down<REMOTE_WITH_BACKEND_MARKER_ALIGN>(get_remote_and_sizeclass()));
    }
    ...
    [[nodiscard]] SNMALLOC_FAST_PATH sizeclass_t get_sizeclass() const {
        ...
        return sizeclass_t::from_raw(static_cast<size_t>(get_remote_and_sizeclass())
    ...
    public:
         ...
         */
        constexpr BackendStateWordRef(uintptr_t* v)
            : val(v) {}
         ...
         * using this.
         */
        BackendStateWordRef& operator=(const BackendStateWordRef& other) = default;
        ...
        BackendStateWordRef& operator=(uintptr_t v) {
            SNMALLOC_ASSERT_MSG(((v & BACKEND_RESERVED_MASK) == 0),
                                "The back end is not permitted to use the low bits in the meta "
                                "entry. ({} & {}) == {}.",
                                v, BACKEND_RESERVED_MASK, (v & BACKEND_RESERVED_MASK));
    ...
    BackendStateWordRef get_backend_word(Word w) {
        if (!is_backend_owned()) {
            SNMALLOC_ASSERT_MSG(is_unowned(),
...
public:
     ...
     */
    SNMALLOC_NO_UNIQUE_ADDRESS freelist::Builder<mitigations(random_preserve)> free_queue;
     ...
     */
    void initialise(smallsizeclass_t sizeclass, address_t slab, const FreeListKey& key) {
        static_assert(std::is_base_of<FrontendSlabMetadata_Trait, BackendType>::value,
        ...
        new (&client_meta_) typename ClientMeta::StorageType[get_client_storage_count(sizeclass)];
    }
     ...
     */
    SNMALLOC_FAST_PATH bool set_sleeping(smallsizeclass_t sizeclass, uint16_t remaining) {
    ...
    alloc_free_list(Domesticator domesticate, FrontendSlabMetadata* meta, freelist::Iter<>& fast_free_list,
                    LocalEntropy& entropy, smallsizeclass_t sizeclass) {
        auto& key = freelist::Object::key_root;

        std::remove_reference_t<decltype(fast_free_list)> tmp_fl;

        auto remaining = meta->free_queue.close(tmp_fl, key, meta->as_key_tweak());
    ...
    [[nodiscard]] SNMALLOC_FAST_PATH static address_t as_key_tweak(address_t self) {
    ...
    static size_t get_extra_bytes(sizeclass_t sizeclass) {
        if (sizeclass.is_small()) {
            ...
            // start with.
            return (get_client_storage_count(sizeclass.as_small()) - 1) * sizeof(typename ClientMeta::StorageType);
        }
...
class FrontendMetaEntry : public MetaEntryBase {
     ...
     */
    static_assert(std::is_convertible_v<SlabMetadataType, FrontendSlabMetadata_Trait>,
...
public:
    ...
    FrontendMetaEntry(SlabMetadata* meta, uintptr_t remote_and_sizeclass)
        ...
        SNMALLOC_ASSERT_MSG((REMOTE_BACKEND_MARKER & remote_and_sizeclass) == 0,
