...
struct alignas(REMOTE_MIN_ALIGN) FreeListMPSCQ {
    ...
    void invariant() {
        SNMALLOC_ASSERT((address_cast(front.load()) == address_cast(&stub)) || (back != nullptr));
    }
    ...
    inline bool can_dequeue(Domesticator_head domesticate_head, Domesticator_queue domesticate_queue) {
        return domesticate_head(front.load())->atomic_read_next(Key, Key_tweak, domesticate_queue) != nullptr;
    }
    ...
    void enqueue(freelist::HeadPtr first, freelist::HeadPtr last, Domesticator_head domesticate_head) {
        invariant();
        ...
        freelist::QueuePtr prev = back.exchange(capptr_rewild(last), std::memory_order_acq_rel);

        if (SNMALLOC_LIKELY(prev != nullptr)) {
            freelist::Object::atomic_store_next(domesticate_head(prev), first, Key, Key_tweak);
            return;
        }
    ...
    void dequeue(Domesticator_head domesticate_head, Domesticator_queue domesticate_queue, Cb cb) {
        ...
        while (address_cast(curr) != address_cast(b)) {
            freelist::HeadPtr next = curr->atomic_read_next(Key, Key_tweak, domesticate_queue);
