...
private:
    ...
    SNMALLOC_SLOW_PATH capptr::Alloc<void> alloc_not_small(size_t size) {
        ...
        return check_init([&](CoreAlloc* core_alloc) {
            ...
            auto [chunk, meta] = Config::Backend::alloc_chunk(
                core_alloc->get_backend_local_state(), large_size_to_chunk_size(size),
                PagemapEntry::encode(core_alloc->public_state(), size_to_sizeclass_full(size)),
            ...
            if (meta != nullptr) {
                meta->initialise_large(address_cast(chunk), freelist::Object::key_root);
                core_alloc->laden.insert(meta);
            }

            if (zero_mem == YesZero && chunk.unsafe_ptr() != nullptr) {
                Config::Pal::template zero<false>(chunk.unsafe_ptr(), bits::next_pow2(size));
            }
    ...
    SNMALLOC_FAST_PATH capptr::Alloc<void> small_alloc(size_t size) {
        ...
        auto slowpath = [&](smallsizeclass_t sizeclass, freelist::Iter<>* fl) SNMALLOC_FAST_PATH_LAMBDA {
            if (SNMALLOC_LIKELY(core_alloc != nullptr)) {
                return core_alloc->handle_message_queue(
                    [](CoreAlloc* core_alloc, smallsizeclass_t sizeclass, freelist::Iter<>* fl) {
     ...
     */
    SNMALLOC_SLOW_PATH void dealloc_remote_slow(const PagemapEntry& entry, capptr::Alloc<void> p) {
        if (core_alloc != nullptr) {
#ifdef SNMALLOC_TRACING
            message<1024>("Remote dealloc post {} ({}, {})", p.unsafe_ptr(), alloc_size(p.unsafe_ptr()),
                          address_cast(entry.get_slab_metadata()));
#endif
            local_cache.remote_dealloc_cache.template dealloc<sizeof(CoreAlloc)>(entry.get_slab_metadata(), p,
                                                                                 &local_cache.entropy);
            post_remote_cache();
            return;
        }
     ...
     */
    template<typename T>
    SNMALLOC_FAST_PATH auto call_ensure_init(T*, int) -> decltype(T::ensure_init()) {
        T::ensure_init();
    }
...
public:
    ...
    SNMALLOC_FAST_PATH ALLOCATOR void* alloc(size_t size) {
        ...
        // slow path.
        if (SNMALLOC_LIKELY((size - 1) <= (sizeclass_to_size(NUM_SMALL_SIZECLASSES - 1) - 1))) {
    ...
    SNMALLOC_FAST_PATH void dealloc_cheri_checks(void* p) {
         ...
         */
        snmalloc_check_client(mitigations(cheri_checks), !__builtin_cheri_sealed_get(p),
        ...
        snmalloc_check_client(mitigations(cheri_checks), (__builtin_cheri_perms_get(p) & reqperm) == reqperm,
         ...
         * elide this test.
         */
        snmalloc_check_client(mitigations(cheri_checks), __builtin_cheri_tag_get(p),
         ...
         */
        snmalloc_check_client(mitigations(cheri_checks), __builtin_cheri_length_get(p) > 0,
...
#  endif
         ...
         */
        capptr::Alloc<void> p_tame = capptr_domesticate<Config>(core_alloc->backend_state_ptr(), p_wild);

        const PagemapEntry& entry = Config::Backend::get_metaentry(address_cast(p_tame));
    ...
    SNMALLOC_SLOW_PATH void dealloc_remote(const PagemapEntry& entry, capptr::Alloc<void> p_tame) {
        RemoteAllocator* remote = entry.get_remote();
        if (SNMALLOC_LIKELY(remote != nullptr)) {
            dealloc_cheri_checks(p_tame.unsafe_ptr());

            // Detect double free of large allocations here.
            snmalloc_check_client(mitigations(sanity_checks), !entry.is_backend_owned(), "Memory corruption detected");

            // Check if we have space for the remote deallocation
            if (local_cache.remote_dealloc_cache.reserve_space(entry)) {
                local_cache.remote_dealloc_cache.template dealloc<sizeof(CoreAlloc)>(entry.get_slab_metadata(), p_tame,
                                                                                     &local_cache.entropy);
#  ifdef SNMALLOC_TRACING
                message<1024>("Remote dealloc fast {} ({}, {})", address_cast(p_tame), alloc_size(p_tame.unsafe_ptr()),
                              address_cast(entry.get_slab_metadata()));
#  endif
                return;
            }
        ...
        snmalloc_check_client(mitigations(sanity_checks), p_tame == nullptr, "Not allocated by snmalloc.");
...
#  endif
    ...
    void check_size(void* p, size_t size) {
        ...
        if constexpr (mitigations(sanity_checks)) {
            ...
            auto pm_sc   = Config::Backend::get_metaentry(address_cast(p)).get_sizeclass();
            auto rsize   = sizeclass_full_to_size(sc);
            auto pm_size = sizeclass_full_to_size(pm_sc);
            snmalloc_check_client(mitigations(sanity_checks), (sc == pm_sc) || (p == nullptr),
                                  "Dealloc rounded size mismatch: {} != {}", rsize, pm_size);
        } else {
            UNUSED(p, size);
        }
#endif
    ...
    SNMALLOC_FAST_PATH size_t alloc_size(const void* p_raw) {
        ...
        const PagemapEntry& entry = Config::Backend::get_metaentry(address_cast(p_raw));
    ...
    typename Config::ClientMeta::DataRef get_client_meta_data(void* p) {
        const PagemapEntry& entry = Config::Backend::get_metaentry(address_cast(p));
        ...
        if (SNMALLOC_UNLIKELY(meta_slab == nullptr)) {
            error("Cannot access meta-data for non-snmalloc object in writable form!");
        }
     ...
     * pointer.
     */
    std::add_const_t<typename Config::ClientMeta::DataRef> get_client_meta_data_const(void* p) {
        const PagemapEntry& entry = Config::Backend::template get_metaentry<true>(address_cast(p));
    ...
    size_t remaining_bytes(address_t p) {
        ...
        const PagemapEntry& entry = Config::Backend::template get_metaentry<true>(p);

        auto sizeclass = entry.get_sizeclass();
        return snmalloc::remaining_bytes(sizeclass, p);
#else
        return reinterpret_cast<size_t>(std::numeric_limits<decltype(p)>::max() - p);
#endif
    }
