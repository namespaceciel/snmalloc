 ...
 */
template<typename T>
SNMALLOC_FAST_PATH auto call_is_initialised(T*, int) -> decltype(T::is_initialised()) {
 ...
 */
template<SNMALLOC_CONCEPT(IsConfigDomestication) Config, typename T, SNMALLOC_CONCEPT(capptr::IsBound) B>
constexpr SNMALLOC_FAST_PATH auto has_domesticate(int) -> std::enable_if_t<
    std::is_same_v<decltype(Config::capptr_domesticate(std::declval<typename Config::LocalState*>(),
                                                       std::declval<CapPtr<T, B>>())),
                   CapPtr<T, typename B::template with_wildness<capptr::dimension::Wildness::Tame>>>,
    bool> {
    return true;
}
 ...
 */
template<SNMALLOC_CONCEPT(IsConfig) Config, typename T, SNMALLOC_CONCEPT(capptr::IsBound) B>
 ...
 */
template<SNMALLOC_CONCEPT(IsConfig) Config, typename T, SNMALLOC_CONCEPT(capptr::IsBound) B>
SNMALLOC_FAST_PATH_INLINE auto capptr_domesticate(typename Config::LocalState* ls, CapPtr<T, B> p) {
    static_assert(!detail::has_domesticate<Config, T, B>(0) || Config::Options.HasDomesticate,
