...
class RemoteDeallocCacheBatching {
    ...
    SNMALLOC_FAST_PATH size_t ring_set(typename Config::PagemapEntry::SlabMetadata* meta) {
        // See https://github.com/skeeto/hash-prospector for choice of constant
        return ((meta->as_key_tweak() * 0x7EFB352D) >> 16) & bits::mask_bits(DEALLOC_BATCH_RING_SET_BITS);
    }
    ...
    SNMALLOC_FAST_PATH void close_one_pending(Forward forward, size_t ix) {
        auto rmsg = BatchedRemoteMessage::mk_from_freelist_builder(
            open_builder[ix], freelist::Object::key_root,
    ...
    SNMALLOC_FAST_PATH void init_one_pending(size_t ix, typename Config::PagemapEntry::SlabMetadata* meta) {
        open_builder[ix].init(0, freelist::Object::key_root,
                              Config::PagemapEntry::SlabMetadata::as_key_tweak(open_meta[ix]));
...
public:
    template<typename Forward>
    SNMALLOC_FAST_PATH void dealloc(typename Config::PagemapEntry::SlabMetadata* meta, freelist::HeadPtr r,
                                    LocalEntropy* entropy, Forward forward) {
        ...
        for (size_t ix_way = 0; ix_way < DEALLOC_BATCH_RING_ASSOC; ix_way++) {
            ...
            if (address_cast(meta) == open_meta[ix]) {
                open_builder[ix].add(r, freelist::Object::key_root, meta->as_key_tweak());
        ...
        open_builder[victim_ix].add(r, freelist::Object::key_root, meta->as_key_tweak());
    }
...
struct RemoteDeallocCacheNoBatching {
    ...
    template<typename Forward>
    SNMALLOC_FAST_PATH void dealloc(typename Config::PagemapEntry::SlabMetadata*, freelist::HeadPtr r,
                                    LocalEntropy* entropy, Forward forward) {
        UNUSED(entropy);

        auto& entry = Config::Backend::get_metaentry(address_cast(r));
        forward(entry.get_remote()->trunc_id(), SingletonRemoteMessage::emplace_in_alloc(r.as_void()));
    }
};

template<typename Config>
using RemoteDeallocCacheBatchingImpl =
    std::conditional_t<(DEALLOC_BATCH_RINGS > 0), RemoteDeallocCacheBatching<Config, DEALLOC_BATCH_RINGS>,
...
struct RemoteDeallocCache {
    ...
    inline size_t get_slot(size_t i, size_t r) {
        constexpr size_t initial_shift = bits::next_pow2_bits_const(allocator_size);
        // static_assert(
        //   initial_shift >= 8,
    ...
    SNMALLOC_FAST_PATH bool reserve_space(const Entry& entry, uint16_t n = 1) {
        ...
        auto size = n * static_cast<int64_t>(sizeclass_full_to_size(entry.get_sizeclass()));
    ...
    template<size_t allocator_size>
    SNMALLOC_FAST_PATH void forward(RemoteAllocator::alloc_id_t target_id, capptr::Alloc<RemoteMessage> msg) {
        list[get_slot<allocator_size>(target_id, 0)].add(RemoteMessage::to_message_link(msg),
                                                         RemoteAllocator::key_global, NO_KEY_TWEAK);
    }

    template<size_t allocator_size>
    SNMALLOC_FAST_PATH void dealloc(typename Config::PagemapEntry::SlabMetadata* meta, capptr::Alloc<void> p,
                                    LocalEntropy* entropy) {
        SNMALLOC_ASSERT(initialised);

        auto r = freelist::Object::make<capptr::bounds::AllocWild>(p);

        batching.dealloc(meta, r, entropy,
                         [this](RemoteAllocator::alloc_id_t target_id, capptr::Alloc<RemoteMessage> msg) {
                             forward<allocator_size>(target_id, msg);
                         });
    }

    template<size_t allocator_size>
    bool post(typename Config::LocalState* local_state, RemoteAllocator::alloc_id_t id) {
        ...
        batching.close_all([this](RemoteAllocator::alloc_id_t target_id, capptr::Alloc<RemoteMessage> msg) {
            forward<allocator_size>(target_id, msg);
        });
        ...
        while (true) {
            ...
            for (size_t i = 0; i < REMOTE_SLOTS; i++) {
                ...
                if (!list[i].empty()) {
                    ...
                    const auto& entry    = Config::Backend::get_metaentry(address_cast(first_));
                    ...
                    snmalloc_check_client(mitigations(sanity_checks), !entry.is_backend_owned(),
