...
template<class T>
class PoolState {
    template<typename TT, SNMALLOC_CONCEPT(Constructable<TT>) Construct, PoolState<TT>& get_state()>
    friend class Pool;
 ...
 * this use case.
 */
template<typename T, SNMALLOC_CONCEPT(Constructable<T>) ConstructT = DefaultConstruct<T>,
