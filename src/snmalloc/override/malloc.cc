...
SNMALLOC_EXPORT void* SNMALLOC_NAME_MANGLE(reallocarray)(void* ptr, size_t nmemb, size_t size) {
    return snmalloc::libc::reallocarray(ptr, nmemb, size);
}
#endif

#if !defined(SNMALLOC_NO_REALLOCARR)
SNMALLOC_EXPORT int SNMALLOC_NAME_MANGLE(reallocarr)(void* ptr, size_t nmemb, size_t size) {
    return snmalloc::libc::reallocarr(ptr, nmemb, size);
}
#endif

SNMALLOC_EXPORT void* SNMALLOC_NAME_MANGLE(memalign)(size_t alignment, size_t size) {
    return snmalloc::libc::memalign(alignment, size);
}

SNMALLOC_EXPORT void* SNMALLOC_NAME_MANGLE(aligned_alloc)(size_t alignment, size_t size) {
    return snmalloc::libc::aligned_alloc(alignment, size);
}

SNMALLOC_EXPORT int SNMALLOC_NAME_MANGLE(posix_memalign)(void** memptr, size_t alignment, size_t size) {
...
SNMALLOC_EXPORT void* SNMALLOC_NAME_MANGLE(pvalloc)(size_t size) {
    return snmalloc::libc::memalign(OS_PAGE_SIZE, (size + OS_PAGE_SIZE - 1) & ~(OS_PAGE_SIZE - 1));
}
...
SNMALLOC_EXPORT void (*SNMALLOC_NAME_MANGLE(__free_hook))(void* ptr)                  = &SNMALLOC_NAME_MANGLE(free);
SNMALLOC_EXPORT void* (*SNMALLOC_NAME_MANGLE(__malloc_hook))(size_t size)             = &SNMALLOC_NAME_MANGLE(malloc);
SNMALLOC_EXPORT void* (*SNMALLOC_NAME_MANGLE(__realloc_hook))(void* ptr, size_t size) = &SNMALLOC_NAME_MANGLE(realloc);
SNMALLOC_EXPORT void* (*SNMALLOC_NAME_MANGLE(__memalign_hook))(size_t alignment,
                                                               size_t size)           = &SNMALLOC_NAME_MANGLE(memalign);
#endif
}
