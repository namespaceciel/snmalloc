...
public:
     ...
     * JEMalloc APIs.
     */
    constexpr JEMallocFlags(int flags)
        : flags(flags) {}
 ...
 */
int SNMALLOC_NAME_MANGLE(mallctlbymib)(const size_t*, size_t, void*, size_t*, void*, size_t) {
    return ENOENT;
}
 ...
 * now, this is always implemented to return an error.
 */
SNMALLOC_EXPORT int SNMALLOC_NAME_MANGLE(mallctl)(const char*, void*, size_t*, void*, size_t) {
    return ENOENT;
}
 ...
 */
int SNMALLOC_NAME_MANGLE(allocm)(void** ptr, size_t* rsize, size_t size, int flags) {
 ...
 * bytes.
 */
int SNMALLOC_NAME_MANGLE(rallocm)(void** ptr, size_t* rsize, size_t size, size_t extra, int flags) {
    auto f          = JEMallocFlags(flags);
    ...
    void* p = f.should_zero() ? a.alloc<YesZero>(alloc_size) : a.alloc(alloc_size);
 ...
 */
SNMALLOC_EXPORT void* SNMALLOC_NAME_MANGLE(rallocx)(void* ptr, size_t size, int flags) {
    auto f = JEMallocFlags(flags);
    size   = f.aligned_size(size);
