...
public:
    ...
    static_assert(MetaEntryBase::is_backend_allowed_value(MetaEntryBase::Word::One, RED_BIT));
    static_assert(MetaEntryBase::is_backend_allowed_value(MetaEntryBase::Word::Two, RED_BIT));
    ///@}
    ...
    static bool can_consolidate(Contents k, size_t size) {
        ...
        auto& entry = Pagemap::template get_metaentry_mut<false>(address_cast(larger));
 ...
 * for
 */
template<size_t REFILL_SIZE_BITS, size_t MAX_SIZE_BITS, SNMALLOC_CONCEPT(IsWritablePagemap) Pagemap,
         size_t MIN_REFILL_SIZE_BITS = 0>
class LargeBuddyRange {
    static_assert(REFILL_SIZE_BITS <= MAX_SIZE_BITS, "REFILL_SIZE_BITS > MAX_SIZE_BITS");
    static_assert(MIN_REFILL_SIZE_BITS <= REFILL_SIZE_BITS, "MIN_REFILL_SIZE_BITS > REFILL_SIZE_BITS");
...
public:
    ...
    class Type : public ContainsParent<ParentRange> {
        ...
        std::enable_if_t<exists> parent_dealloc_range(capptr::Arena<void> base, size_t size) {
            static_assert(MAX_SIZE_BITS != (bits::BITS - 1), "Don't set SFINAE parameter");
        ...
        void add_range(capptr::Arena<void> base, size_t length) {
            range_to_pow_2_blocks<MIN_CHUNK_BITS>(base, length, [this](capptr::Arena<void> base, size_t align, bool) {
                auto overflow = capptr::Arena<void>::unsafe_from(
                    reinterpret_cast<void*>(buddy_large.add_block(base.unsafe_uintptr(), align)));
        ...
        capptr::Arena<void> refill(size_t size) {
            ...
            while (needed_size <= refill_size) {
                ...
                if (refill != nullptr) {
                    ...
                    static_assert((REFILL_SIZE < bits::one_at_bit(MAX_SIZE_BITS)) || ParentRange::Aligned,
    ...
    public:
        ...
        static_assert(std::is_same_v<typename ParentRange::ChunkBounds, ChunkBounds>);
        ...
        capptr::Arena<void> alloc_range(size_t size) {
            ...
            auto result = capptr::Arena<void>::unsafe_from(reinterpret_cast<void*>(buddy_large.remove_block(size)));
        ...
        void dealloc_range(capptr::Arena<void> base, size_t size) {
            ...
            auto overflow = capptr::Arena<void>::unsafe_from(
                reinterpret_cast<void*>(buddy_large.add_block(base.unsafe_uintptr(), size)));
            dealloc_overflow(overflow);
        }
    };
};
