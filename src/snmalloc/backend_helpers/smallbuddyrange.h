...
public:
    ...
    static void set(Handle ptr, Contents r) {
        ...
        if (r == nullptr) {
            *ptr = CapPtr<FreeChunk<bounds>, bounds>::unsafe_from(
            ...
            // Preserve lower bit.
            *ptr = pointer_offset(r, (address_cast(*ptr) & MASK)).template as_static<FreeChunk<bounds>>();
        }
    }
    ...
    static void set_red(Contents k, bool new_is_red) {
        if (new_is_red != is_red(k)) {
            ...
            if (new_is_red) {
                if (old_addr == nullptr) {
                    *r = CapPtr<FreeChunk<bounds>, bounds>::unsafe_from(reinterpret_cast<FreeChunk<bounds>*>(MASK));
                } else {
                    *r = pointer_offset(old_addr, MASK).template as_static<FreeChunk<bounds>>();
                }
            } else {
                *r = old_addr;
            }
    ...
    static Contents buddy(Contents k, size_t size) {
        ...
        return pointer_offset(base, offset).template as_static<FreeChunk<bounds>>();
    }
...
struct SmallBuddyRange {
    ...
    private:
        ...
        static constexpr size_t MIN_BITS = bits::next_pow2_bits_const(sizeof(FreeChunk<ChunkBounds>));
        ...
        void add_range(CapPtr<void, ChunkBounds> base, size_t length) {
            range_to_pow_2_blocks<MIN_BITS>(base, length, [this](CapPtr<void, ChunkBounds> base, size_t align, bool) {
                if (align < MIN_CHUNK_SIZE) {
                    CapPtr<void, ChunkBounds> overflow =
                        buddy_small.add_block(base.template as_reinterpret<FreeChunk<ChunkBounds>>(), align)
