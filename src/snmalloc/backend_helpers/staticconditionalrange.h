...
struct StaticConditionalRange {
    ...
    class Type : public ContainsParent<Pipe<ParentRange, OptionalRange>> {
        ...
        inline static bool disable_range_{false};

    public:
        // Both parent and grandparent must be aligned for this range to be
        // aligned.
        static constexpr bool Aligned = ActualParentRange::Aligned && ParentRange::Aligned;

        // Both parent and grandparent must be aligned for this range to be
        // concurrency safe.
        static constexpr bool ConcurrencySafe = ActualParentRange::ConcurrencySafe && ParentRange::ConcurrencySafe;

        using ChunkBounds = typename ActualParentRange::ChunkBounds;

        static_assert(std::is_same_v<ChunkBounds, typename ParentRange::ChunkBounds>,
