...
struct BasicAuthmap {
    static_assert(std::is_same_v<capptr::Arena<void>, typename ConcreteMap::EntryType>,
                  "BasicAuthmap's ConcreteMap must have capptr::Arena<void> element type!");

private:
    SNMALLOC_REQUIRE_CONSTINIT
    inline static ConcreteMap concreteAuthmap;
...
public:
    ...
    static SNMALLOC_FAST_PATH void register_range(capptr::Arena<void> base, size_t size) {
        concreteAuthmap.register_range(address_cast(base), size);

        address_t base_addr = address_cast(base);
        for (address_t a = base_addr; a < base_addr + size; a += ConcreteMap::GRANULARITY) {
    ...
    template<bool potentially_out_of_range = false>
    static SNMALLOC_FAST_PATH capptr::Arena<void> amplify(capptr::Alloc<void> c) {
        return Aal::capptr_rebound(concreteAuthmap.template get<potentially_out_of_range>(address_cast(c)), c);
    }
};
...
using DefaultAuthmap = std::conditional_t<aal_supports<StrictProvenance>, BasicAuthmap<CA>, DummyAuthmap>;
